...
export const EVENTS = [
  ...
  // R17/8.2: patron damage stacking trimmed ~20% (3 patrons/run each multiplied independently)
  { id: 'p_gambler', name: '阿奇·卡拉斯', role: '賭徒', title: '孤注一擲', icon: 'patron_gambler',
    desc: '傷害 +10%；命中敵人時有 5% 機率爆出額外金幣。',
    apply: (s) => { s.player.stats.damageMult *= 1.10; s.player.hooks.hit.push((e, dmg, w) => { if (Math.random() < 0.05) { const g = 2 + Math.floor((s.threat || 1) * 0.6); s.run.gold += g; w.particles.text(e.x, e.y - 10, '+' + g + '金', { color: P.goldL, size: 10, weight: '800' }); } }); } },
  { id: 'p_midas', name: '金手指·麥達斯', role: '黃金狂人', title: '點石成金', icon: 'patron_midas',
    desc: '金幣越多傷害越高（上限 +24%）；金幣獲取 +12%。',
    apply: (s) => { s.player.stats.goldMult *= 1.12; const bonus = Math.min(0.24, (s.run.gold || 0) / 2000 * 0.24); s.player.stats.damageMult *= 1 + bonus; s.banner = '點石成金：依現有金幣獲得 +' + Math.round(bonus * 100) + '% 傷害'; s.bannerT = 2.4; } },
  { id: 'p_pyro', name: '焚心·伊格尼斯', role: '炎術士', title: '燎原之印', icon: 'patron_pyro',
    desc: '所有攻擊命中有 30% 機率點燃敵人（灼燒）。',
    apply: (s) => { s.player.hooks.hit.push((e, dmg, w) => { if (Math.random() < 0.30) applyStatus(e, 'burn', w); }); s.player.stats.damageMult *= 1.04; } },
  ...
  { id: 'p_mage', name: '星河·瑟蕾娜', role: '大法師', title: '奧術精通', icon: 'patron_mage',
    desc: '範圍 +22%、傷害 +10%（脆皮代價：生命 -10）。',
    apply: (s) => { const st = s.player.stats; st.area = (st.area || 1) * 1.22; st.damageMult *= 1.10; st.maxHp = Math.max(20, st.maxHp - 10); s.player.hp = Math.min(s.player.hp, st.maxHp); } },
  { id: 'p_berserker', name: '血斧·卡爾戈', role: '狂戰士', title: '嗜血狂怒', icon: 'patron_berserker',
    desc: '傷害 +28%，但減傷 -3。', apply: (s) => { s.player.stats.damageMult *= 1.28; s.player.stats.defense -= 3; } },
  ...
  { id: 'p_engineer', name: '發條·蓋茲', role: '工匠', title: '多重彈幕', icon: 'patron_engineer',
    desc: '投射物 +1、穿透 +1、傷害 +5%。',
    apply: (s) => { const st = s.player.stats; st.projCountAdd = (st.projCountAdd || 0) + 1; st.pierceAdd = (st.pierceAdd || 0) + 1; st.damageMult *= 1.05; } },
  { id: 'p_merchant', name: '商隊·賽門', role: '行商', title: '財富湧流', icon: 'patron_merchant',
    desc: '立即 +600 金幣、+90 魂晶；幸運 +0.3。',
    apply: (s) => { s.run.gold += 600; s.run.shards += 90; s.player.stats.luck = (s.player.stats.luck || 0) + 0.3; } },   // R17/8.3: instant gold halved
