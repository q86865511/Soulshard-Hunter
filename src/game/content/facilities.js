...
const cost = (base, growth = 1.8) => (lvl) => Math.round(base * Math.pow(growth, lvl));

F({
  id: 'f_shrine', name: '生命神龕', desc: '每次出擊 生命上限 +12/級', maxLevel: 5, icon: 'facility_f_shrine',
  cost: cost(120), applyRun: (run, l) => { run.stats.maxHp += 12 * l; },
});
F({
  id: 'f_bank', name: '金庫', desc: '金幣獲取 +6%/級', maxLevel: 5, icon: 'facility_f_bank',
  cost: cost(140), applyRun: (run, l) => { run.stats.goldMult *= 1 + 0.06 * l; },
});
F({
...
F({
  id: 'f_dojo', name: '修練場', desc: '每次出擊 額外 +1 升級選擇/級', maxLevel: 4, icon: 'facility_f_dojo',
  cost: cost(180), applyRun: (run, l) => { run.startBonusLevels = (run.startBonusLevels || 0) + l; },
});
F({
  id: 'f_altar', name: '魂晶祭壇', desc: '每次出擊 起始魂晶 +8/級', maxLevel: 5, icon: 'facility_f_altar',
  cost: cost(130), applyRun: (run, l) => { run.shards += 8 * l; },
});
F({
