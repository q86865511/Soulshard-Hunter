...
defineAnim('evt_bomber', 16, 15, 4, (p, f) => {
  ...
  // R28 W3-A1: canvas already standard-tier (16x15) — value-tier gap only.
...
defineAnim('evt_bomb', 14, 14, 3, (p, f) => {
  // R28 W3-A1: canvas already swarm-tier (14x14) — widened the base disc + crystal
  // body (was 49% fill, thin spike on a mostly-empty 14x14 canvas) to clear the
  // >=55% fill floor without changing the "mine with a warning crystal" concept.
  const pulse = [0, 1, 2][f % 3]; const cy = 9;
  p.ellipse(7, cy + 2, 6, 3, darken(P.ink2, 0.1));                          // base disc
  p.ellipse(7, cy + 1.6, 5, 2.4, P.ink2);
  ...
  p.rect(4, cy - 3, 6, 5, darken(core, 0.25));                               // crystal base (tapered by the cap above)
  p.rect(5, cy - 5, 4, 3, core);
  p.vline(cy - 4, cy + 1, 7, lighten(core, 0.3));                            // bright seam
  p.px(7, cy - 6, lighten(core, 0.45));
  if (pulse === 2) { p.sparkle(7, cy - 5, '#ffffff'); p.glow(7, cy - 3, 3, withAlpha(P.laser, 0.4)); }
  p.rimLight(P.rim, 0.4);
  p.shadeBottom(0.2, 9);
  p.outline(P.ink);
}, { anchor: [7, 13], fps: 4 });
...
defineAnim('evt_boulder', 18, 18, 4, (p, f) => {
  ...
  // R28 W3-A1: canvas already large-tier (18x18) — value-tier gap only.
  p.rimLight(P.rim, 0.4);
...
defineAnim('evt_goblin', 16, 16, 4, (p, f) => {
  // R28 W3-A1: standard-tier 16x16 canvas (was 16x17); -0.5px y-shift to recentre.
  p.ctx.save(); p.ctx.translate(0, -0.5);
  // R28 W3-A1: tunic/legs/sack thickened to hold >=55% fill on the standard-tier
  // canvas (was 16x17, thin-frame goblin at 49% fill).
  const run = [0, 1, 0, -1][f % 4]; const by = 10;
  const skin = mix(P.green, P.leaf, 0.45), skinD = darken(skin, 0.2);
  p.rect(4 + run, by + 4, 3, 3, skinD); p.rect(9 - run, by + 4, 3, 3, skinD);   // scrambling legs
  p.rect(4, by, 8, 5, mix(P.leather, P.woodD, 0.4));                        // ragged tunic
  p.hline(4, 11, by + 2, P.woodD);
  ...
  p.ellipse(sx, sy, 4.6, 5, P.leather); p.ellipse(sx, sy - 0.4, 3.8, 4.2, lighten(P.leather, 0.12));
