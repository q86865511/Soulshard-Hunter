...
// ---- R18 / B5: six former-orphan heroes promoted to playable. Each maps to a unique
// body archetype already registered in art/heroes.js HERO_MAP (berserker/mage/scout/
// valkyrie/necromancer/stormpriest) — drawHeroBody recolours it. Stat fields all exist
// in makeBaseStats(); unlock prices align with the R17 economy; NPC-affinity-free. ----
C({ id: 'g_vanguard', name: '鐵血先鋒', desc: '坦克：生命 +50、減傷 +4、擊退 +25%、移速 -8%、回復 +0.4。起始武器：魂鞭。', sprite: 'char_g_vanguard', startWeapon: 'w_whip',
  art: { cloak: P.blood, cloakD: P.redD, cloakL: P.red, trim: P.steelL, eye: P.emberL },
  passive: (s) => { s.maxHp += 50; s.defense += 4; s.knockbackMult *= 1.25; s.speed *= 0.92; s.hpRegen += 0.4; }, unlock: { type: 'gold', cost: 500 } });

C({ id: 'g_arcanist', name: '奧術秘士', desc: '玻璃法師：傷害 +18%、範圍 +30%、經驗 +15%、生命 -15。起始武器：震爆波。', sprite: 'char_g_arcanist', startWeapon: 'w_nova',
  art: { cloak: P.mana, cloakD: P.purpleD, cloakL: P.manaL, trim: P.shardL, eye: P.shardL },
  passive: (s) => { s.maxHp -= 15; s.damageMult *= 1.18; s.area *= 1.3; s.xpMult *= 1.15; }, unlock: { type: 'gold', cost: 650 } });

C({ id: 'g_ranger', name: '翠林遊俠', desc: '暴擊射手：暴擊 +12%、暴傷 +0.3、射速 +12%、生命 -12。起始武器：魂晶彈。', sprite: 'char_g_ranger', startWeapon: 'w_soulbolt',
  art: { cloak: P.green, cloakD: P.greenD, cloakL: P.greenL, trim: P.gold, eye: P.toxic },
  passive: (s) => { s.critChance += 0.12; s.critMult += 0.3; s.fireRateMult *= 1.12; s.maxHp -= 12; }, unlock: { type: 'achievement', condition: 'survive_600', hint: '單局存活 10 分鐘解鎖' } });

C({ id: 'g_warden', name: '霜鋼守望', desc: '環刃守護：生命 +20、閃避 +12%、投射 +1、傷害 -10%。起始武器：環衛刃。', sprite: 'char_g_warden', startWeapon: 'w_orbit',
  art: { cloak: P.ice, cloakD: P.iceD, cloakL: P.white, trim: P.steelL, eye: P.blueL },
  passive: (s) => { s.maxHp += 20; s.dodge += 0.12; s.projCountAdd += 1; s.damageMult *= 0.9; }, unlock: { type: 'gold', cost: 800 } });

C({ id: 'g_revenant', name: '永夜亡魂', desc: '吸血光環：生命 +25、吸血 +6%、回復 +0.6、範圍 +15%。起始武器：灼蝕光環。', sprite: 'char_g_revenant', startWeapon: 'w_aura',
  art: { cloak: P.purple, cloakD: P.purpleD, cloakL: P.purpleL, trim: P.toxic, eye: P.toxic },
  passive: (s) => { s.maxHp += 25; s.lifesteal += 0.06; s.hpRegen += 0.6; s.area *= 1.15; }, unlock: { type: 'achievement', condition: 'reach_stage_8', hint: '抵達威脅 8 級解鎖' } });

C({ id: 'g_stormcaller', name: '雷霆喚使', desc: '雷暴彈幕：投射 +1、彈速 +20%、傷害 +10%、生命 -20。起始武器：連鎖閃電。', sprite: 'char_g_stormcaller', startWeapon: 'w_lightning',
  art: { cloak: P.blue, cloakD: P.blueD, cloakL: P.blueL, trim: P.emberL, eye: P.emberL },
  passive: (s) => { s.maxHp -= 20; s.projCountAdd += 1; s.projSpeedMult *= 1.2; s.damageMult *= 1.1; }, unlock: { type: 'achievement', condition: 'endless_1200', hint: '無盡模式存活 20 分鐘解鎖' } });
...
function meetsCondition(cond, s) {
  ...
  // R18/B5: endless-survival gate. bestEndlessTime lands in B7; (||0) keeps g_stormcaller
  // simply locked until then — no error pre-B7.
  if ((m = /^endless_(\d+)$/.exec(cond))) return (s.bestEndlessTime || 0) >= +m[1];
  return false;
}
