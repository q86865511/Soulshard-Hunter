...
import { UI, ctxRaw, drawSpriteUI, textWidth, uiClipRound, uiRect, uiText, view } from '../../../engine/renderer.js';
...
export const renderWardrobeMixin = {
  ...
  drawAchievements() {
    ...
    this.achFilterRects(f).forEach((r) => {
      ...
      uiText(FILTERS[r.i], r.x + r.w / 2, r.y + r.h / 2 + 1 * S, { size: UI.FONT_BODY * S, align: 'center', baseline: 'middle', color: on ? acc : P.gray3, weight: '800' });
    });
    ...
    if (!list.length) uiText(fl === 0 ? '暫無接近完成的成就——切到『全部』看看更多' : '（此分類沒有成就）', f.x + f.w / 2, gridTop + 20 * S, { size: UI.FONT_BODY * S, align: 'center', color: P.gray3 });
    list.forEach((a, i) => {
      ...
      uiText(done ? '★' : '☆', x + 12 * S, y + 23 * S, { size: UI.FONT_HEADING * S, color: done ? (hid ? HID : P.goldL) : P.gray2, weight: '900' });
      if (hid) { const pw = textWidth('★隱藏', UI.FONT_CAPTION * S, '800') + 10 * S; uiRect(x + cardW - pw - 8 * S, y + 8 * S, pw, 14 * S, withAlpha(HID, 0.18), { radius: 7 * S, stroke: withAlpha(HID, 0.7), lw: 1 }); uiText('★隱藏', x + cardW - pw / 2 - 8 * S, y + 15 * S, { size: UI.FONT_CAPTION * S, align: 'center', baseline: 'middle', color: HID, weight: '800' }); }
      uiText(name, x + 34 * S, y + 20 * S, { size: UI.FONT_BODY * S, color: done ? '#fff' : P.gray3, weight: '800' });
    ...
    uiText(`已解鎖 ${prog.unlocked} / ${prog.total}　·　▲▼ 滾輪捲動　·　Esc 關閉`, f.x + f.w / 2, f.y + f.h - 14 * S, { size: UI.FONT_BODY * S, align: 'center', color: P.goldL, weight: '600' });
  },
  ...
  drawWardrobeEntry(f) {
    ...
    const door = (r, icon, title, caption, accent) => {
      ...
      uiText(icon, r.x + r.w / 2, r.y + r.h * 0.38, { size: 34 * S, align: 'center', color: accent, weight: '900' });   /* pictogram, exempt from type ramp */
      uiText(title, r.x + r.w / 2, r.y + r.h * 0.62, { size: UI.FONT_HEADING * S, align: 'center', color: hov ? '#fff' : '#eaf2ff', weight: '900' });
      uiText(caption, r.x + r.w / 2, r.y + r.h * 0.62 + 20 * S, { size: UI.FONT_CAPTION * S, align: 'center', color: P.gray3, weight: '600' });
    };
  ...
  drawSkinShop(f) {
    ...
    uiText('◀ 返回', L.back.x + L.back.w / 2, L.back.y + L.back.h / 2 + 1 * S, { size: UI.FONT_BODY * S, align: 'center', baseline: 'middle', color: '#cfe0ff', weight: '600' });
    ...
    if (!dry) {
      ...
      uiText('↻ 下次免費進貨 ' + mm + ':' + String(ss2).padStart(2, '0'), L.reroll.x - 12 * S, L.reroll.y + L.reroll.h / 2 + 1 * S, { size: UI.FONT_CAPTION * S, align: 'right', baseline: 'middle', color: P.gray3, weight: '600' });
      ...
      goldLabel(L.reroll.x + L.reroll.w / 2, L.reroll.y + L.reroll.h / 2 + 1 * S, rerollCost(META), { size: UI.FONT_BODY * S, align: 'center', baseline: 'middle', color: '#fff', weight: '800', prefix: '↺ 重新進貨 ' });
    }
    if (!L.cards.length) {
      uiText(dry ? '已蒐集所有上架造型！' : '本輪沒有進到新貨——重新進貨試試手氣', f.x + f.w / 2, f.y + f.h / 2, { size: UI.FONT_HEADING * S, align: 'center', baseline: 'middle', color: P.goldL, weight: '900' });
      return;
    }
    ...
    for (const cd of L.cards) {
      ...
      uiText((hidden ? '★ ' : '') + sk.name, cd.x + cd.w / 2, cd.y + cd.h - 52 * S, { size: UI.FONT_BODY * S, align: 'center', color: hidden ? P.goldL : '#fff', weight: '800' });
      // char tag + tier pill row
      const tierTxt = TIER[skinTier(sk)] || '';
      uiText(c.name + '　·　' + tierTxt, cd.x + cd.w / 2, cd.y + cd.h - 38 * S, { size: UI.FONT_CAPTION * S, align: 'center', color: TIERCOL[skinTier(sk)] || P.gray3, weight: '600' });
      if (owned) uiText('已擁有', cd.x + cd.w / 2, cd.y + cd.h - 16 * S, { size: UI.FONT_BODY * S, align: 'center', color: P.greenL, weight: '800' });
      else {
        const afford = (META.gold || 0) >= pr.price;
        if (pr.onSale) {
          uiText(String(Math.round(pr.base)), cd.x + cd.w / 2 - 34 * S, cd.y + cd.h - 16 * S, { size: UI.FONT_CAPTION * S, align: 'center', color: P.gray2, weight: '600' });
          uiText('特賣8折', cd.x + cd.w / 2, cd.y + cd.h - 28 * S, { size: UI.FONT_CAPTION * S, align: 'center', color: P.emberL, weight: '800' });
        }
        goldLabel(cd.x + cd.w / 2 + (pr.onSale ? 10 * S : 0), cd.y + cd.h - 16 * S, pr.price, { size: UI.FONT_BODY * S, align: 'center', color: afford ? P.goldL : P.redL, weight: '900' });
      }
    }
    ...
    uiText('🏷 特賣輪替剩 ' + dd + ' 天 ' + hh + ' 時（僅普通/豪華 8 折，隱藏款恕不打折）　·　Esc 返回', f.x + f.w / 2, f.y + f.h - 14 * S, { size: UI.FONT_BODY * S, align: 'center', color: P.gray3 });
  },
  drawWardrobeChars(f) {
    const S = f.S; const mx = mouse.x * view.dpr, my = mouse.y * view.dpr;
    ...
    uiText('◀ 返回', bk.x + bk.w / 2, bk.y + bk.h / 2 + 1 * S, { size: UI.FONT_BODY * S, align: 'center', baseline: 'middle', color: '#cfe0ff', weight: '600' });
    ...
    for (const card of L.cards) {
      ...
      uiText(c.name, card.x + card.w / 2, card.y + card.h - 22 * S, { size: UI.FONT_BODY * S, align: 'center', color: '#fff', weight: '800' });
      uiText('已持有 ' + ownedN + ' / ' + SKINS.length, card.x + card.w / 2, card.y + card.h - 8 * S, { size: UI.FONT_CAPTION * S, align: 'center', color: P.shardL, weight: '600' });
    }
    ctx.restore(); this.drawScrollbar(f);
    uiText('滾輪捲動　·　Esc 關閉', f.x + f.w / 2, f.y + f.h - 14 * S, { size: UI.FONT_BODY * S, align: 'center', color: P.gray3 });
  },
  ...
  drawWardrobeSkins(f) {
    ...
    for (const r of L.rows) {
      ...
      uiText((hidden ? '★ ' : '') + (id ? sk.name : '原色'), f.x + 66 * S, r.y + 17 * S, { size: UI.FONT_BODY * S, color: hidden ? P.goldL : '#fff', weight: '800' });
      if (id) uiText(TIER[skinTier(sk)] || '', f.x + 66 * S, r.y + 33 * S, { size: UI.FONT_CAPTION * S, color: P.gray3, weight: '600' });
      const b = r.btn;
      if (equipped) uiText('● 使用中', b.x + b.w / 2, b.y + b.h / 2 + 1 * S, { size: UI.FONT_BODY * S, align: 'center', baseline: 'middle', color: P.shardL, weight: '800' });
      else { const h2 = inside(mx, my, b); uiRect(b.x, b.y, b.w, b.h, withAlpha(h2 ? '#27306a' : '#1b2840', 0.96), { radius: 6 * S, stroke: P.shardL, lw: 1.5 }); uiText('裝備', b.x + b.w / 2, b.y + b.h / 2 + 1 * S, { size: UI.FONT_BODY * S, align: 'center', baseline: 'middle', color: '#cfe0ff', weight: '800' }); }
    }
    ctx.restore(); this.drawScrollbar(f);
    uiText('想要更多造型？回到入口逛「造型商店」　·　Esc 返回', f.x + f.w / 2, f.y + f.h - 14 * S, { size: UI.FONT_BODY * S, align: 'center', color: P.gray3 });
  },
};
