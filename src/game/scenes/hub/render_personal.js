...
export const renderPersonalMixin = {
  ...
  drawPersonal() {
    ...
    for (const tb of this.personalTabRects(f)) {
      ...
      uiText(tb.name, tb.x + tb.w / 2, tb.y + tb.h / 2 + 1 * S, { size: UI.FONT_BODY * S, align: 'center', baseline: 'middle', color: on ? '#fff' : P.gray3, weight: '800' });
    }
    ...
    stats.forEach((st, i) => {
      ...
      uiText(st[0], x, y, { size: UI.FONT_BODY * S, color: P.gray3, weight: '600' });
      uiText(String(st[1]), x + cw - 20 * S, y, { size: UI.FONT_BODY * S, align: 'right', color: '#fff', weight: '800' });
    });
    // collection
    const col = [
    ...
    uiText('收藏', sx, f.y + 96 * S + 6 * 26 * S + 8 * S, { size: UI.FONT_BODY * S, color: P.goldL, weight: '800' });
    col.forEach((cc, i) => {
  ...
  drawBondCodex(f) {
    ...
    const topY = f.y + 60 * S;
    uiText('共 ' + BONDS.length + ' 種羈絆　·　湊齊特定武器／被動組合即可逐階啟動，效果持續整場探索', left, topY, { size: UI.FONT_BODY * S, color: P.gray3, weight: '600' });
    const seenCount = BONDS.filter((b) => seen.has(b.id)).length;
    uiText('已解鎖 ' + seenCount + ' / ' + BONDS.length, right, topY, { size: UI.FONT_BODY * S, align: 'right', color: P.goldL, weight: '800' });
    ...
    for (const b of BONDS) {
      ...
      if (y + h >= viewTop - 4 * S && y <= viewBot + 4 * S) {
        ...
        uiText(b.tag, left + 11 * S, y + 15 * S, { size: UI.FONT_BODY * S, align: 'center', baseline: 'middle', color: isSeen ? '#1a1404' : P.gray3, weight: '900' });
        uiText(b.name, left + 26 * S, y + 17 * S, { size: UI.FONT_BODY * S, color: isSeen ? P.goldL : '#c8cfe8', weight: '900' });
        uiText(isSeen ? '✓ 已解鎖' : '未解鎖', right - 4 * S, y + 17 * S, { size: UI.FONT_CAPTION * S, align: 'right', color: isSeen ? P.greenL : P.gray2, weight: '600' });
        uiText('需求：' + b.goal, left + 26 * S, y + 32 * S, { size: UI.FONT_CAPTION * S, color: P.gray3, weight: '600' });
        b.tiers.forEach((t, k) => {
          const ty = y + 47 * S + k * 15 * S;
          uiText('第 ' + (k + 1) + ' 階（達成 ' + t.at + '）', left + 26 * S, ty, { size: UI.FONT_CAPTION * S, color: isSeen ? '#dfe6ff' : P.gray3, weight: '600' });
          uiText(t.bonusDesc, left + 156 * S, ty, { size: UI.FONT_CAPTION * S, color: isSeen ? P.emberL : P.gray2, weight: '600' });
        });
      }
      y += h;
    }
  ...
  drawSortie() {
    ...
    // R18/B9: 每日挑戰 entry bar — borrowed biome+hero+mutators, click to launch directly.
    {
      ...
      uiText('📅 每日挑戰', db.x + 12 * S, db.y + db.h / 2, { size: UI.FONT_BODY * S, align: 'left', baseline: 'middle', color: P.goldL, weight: '800' });
      uiText(bn + ' · ' + hn + ' · ' + mutNames, db.x + 120 * S, db.y + db.h / 2, { size: UI.FONT_CAPTION * S, align: 'left', baseline: 'middle', color: P.holyL, weight: '600' });
      const best = (META.daily && META.daily.key === dc.key) ? (META.daily.best || 0) : 0;
      uiText(best > 0 ? '最佳 ' + best : '未挑戰', db.x + db.w - 12 * S, db.y + db.h / 2, { size: UI.FONT_CAPTION * S, align: 'right', baseline: 'middle', color: P.gold, weight: '800' });
    }
    ...
    for (const card of L.cards) {
      ...
      if (!unlocked) {
        ...
        if (c.unlock.type === 'gold') goldLabel(card.x + lw2 / 2, card.y + card.h - 6 * S, c.unlock.cost, { size: UI.FONT_CAPTION * S, align: 'center', color: afford ? P.goldL : P.gray3, weight: '600', prefix: '🔒 ' });
        else uiText('🔒 成就解鎖', card.x + lw2 / 2, card.y + card.h - 6 * S, { size: UI.FONT_CAPTION * S, align: 'center', color: P.gray3, weight: '600' });
      } else if (selected) uiText('● 已選', card.x + lw2 / 2, card.y + card.h - 6 * S, { size: UI.FONT_CAPTION * S, align: 'center', color: P.shardL, weight: '800' });
    ...
    uiText(`${this.sortPage + 1} / ${L.pages}`, f.x + f.w / 2, L.pgY + 16 * S, { size: UI.FONT_BODY * S, align: 'center', color: P.gray3, weight: '600' });
    // level row
    const sel = this.curBiome(L);
    uiText('關卡', f.x + 24 * S, L.lvlY - 6 * S, { size: UI.FONT_BODY * S, color: P.gray3, weight: '600' });
    for (const lb of L.lvlButtons) {
      ...
      uiText(lb.b.name, lb.x + lb.w / 2, lb.y + lb.h / 2 + 1 * S, { size: UI.FONT_BODY * S, align: 'center', baseline: 'middle', color: on ? '#fff' : P.gray4, weight: '600' });
      if (cleared > 0) uiText('✓' + cleared, lb.x + lb.w - 5 * S, lb.y + 10 * S, { size: UI.FONT_CAPTION * S, align: 'right', color: P.greenL, weight: '800' });
    }
    ...
    uiText('難度', f.x + 24 * S, L.dY + 17 * S, { size: UI.FONT_BODY * S, color: P.gray3, weight: '600' });
    arrow(L.dPrev, '−', this.selDiff > 0);
    arrow(L.dNext, '+', this.selDiff < topD);
    uiText(isEndless ? '∞ 無盡挑戰' : isStory ? '劇情' : ('難度 ' + this.selDiff + (this.selDiff >= maxD ? ' · 最高可玩' : '')), f.x + f.w / 2, L.dY + 17 * S, { size: UI.FONT_BODY * S, align: 'center', baseline: 'middle', color: isEndless ? P.goldL : isStory ? P.shardL : P.emberL, weight: '800' });   // R17 B15: ∞ (U+221E math glyph, in the CJK font) — ♾ U+267E emoji-rendered as a blob
    ...
    uiText(descTxt, f.x + f.w / 2, L.dY + 33 * S, { size: UI.FONT_CAPTION * S, align: 'center', baseline: 'middle', color: isEndless ? P.goldL : P.gray3 });
    // start
    ...
    uiText('出 擊 狩 獵', L.start.x + L.start.w / 2, L.start.y + L.start.h / 2 + 1 * S, { size: UI.FONT_TITLE * S, align: 'center', baseline: 'middle', color: '#fff', weight: '900' });
  },
  ...
  drawSortieTut() {
    ...
    uiText('出 擊 指 南', view.W / 2, Math.max(20 * S, f.y - 12 * S), { size: UI.FONT_TITLE * S, align: 'center', color: P.goldL, weight: '900', shadowColor: withAlpha('#000', 0.8) });
    const ctx = ctxRaw();
    const callout = (tx, ty, bx, by, label) => {
      const tw = textWidth(label, UI.FONT_BODY * S, '600') + 18 * S, bh = 24 * S;
      ...
      uiText(label, bx, by + 1 * S, { size: UI.FONT_BODY * S, align: 'center', baseline: 'middle', color: '#fff', weight: '600' });
    };
    ...
    uiText('點擊任意處關閉（只出現一次，ESC 選單可重看新手指南）', view.W / 2, f.y + f.h + 18 * S > view.H ? view.H - 10 * S : f.y + f.h + 14 * S, { size: UI.FONT_BODY * S, align: 'center', color: withAlpha(P.goldL, 0.7 + 0.3 * Math.sin(this.t * 5)), weight: '600', shadowColor: withAlpha('#000', 0.8) });
  },
