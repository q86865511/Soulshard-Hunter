...
export const renderSmithMixin = {
  ...
  drawSmith() {
    ...
    uiText(this.hubPriceHint(this.tab === 0 ? 'forgePurchases' : 'facilityPurchases'), f.x + f.w / 2, f.y + f.h - 14 * f.S, { size: UI.FONT_CAPTION * f.S, align: 'center', color: P.gray3, weight: '600' });   // 9.3
  },
  drawForge(f) {
    ...
    for (const r of L.rows) {
      ...
      if (sum) uiText(sum, r.x + r.wd - 6 * S, r.y + 25 * S, { size: UI.FONT_CAPTION * S, align: 'right', color: P.emberL, weight: '600' });
    }
    ctx.restore();
    ...
    if (!sel) { uiText('選擇左側武器進行鍛造', d.x + d.w / 2, d.y + d.h / 2, { size: UI.FONT_BODY * S, align: 'center', baseline: 'middle', color: P.gray3 }); return; }
    ...
    uiText(wdef ? wdef.name : sel, d.x + 56 * S, d.y + 24 * S, { size: UI.FONT_HEADING * S, color: '#fff', weight: '900' });
    uiText('鍛造等級 ' + fdata.level + '/' + FORGE_MAX_LEVEL + '（每級傷害 +8%）', d.x + 56 * S, d.y + 42 * S, { size: UI.FONT_BODY * S, color: P.emberL, weight: '600' });
    ...
    if (b.level) { const can = META.gold >= forgeLevelCost(fdata.level), hov = inside(mx, my, b.level); uiRect(b.level.x, b.level.y, b.level.w, b.level.h, withAlpha(can ? (hov ? '#3a5a2a' : '#26401c') : '#2a2030', 0.96), { radius: 6 * S, stroke: can ? P.greenL : P.gray1, lw: 2 }); goldLabel(b.level.x + b.level.w / 2, b.level.y + b.level.h / 2 + 1 * S, forgeLevelCost(fdata.level), { size: UI.FONT_BODY * S, align: 'center', baseline: 'middle', color: can ? '#fff' : P.gray3, weight: '800', prefix: '強化等級 ' }); }
    else uiText('★ 鍛造等級已滿', d.x + 14 * S, d.y + 110 * S, { size: UI.FONT_BODY * S, color: P.greenL, weight: '800' });
    uiText('特效（最多 ' + FORGE_MAX_EFFECTS + ' 種，已鑲 ' + fdata.effects.length + '）', d.x + 14 * S, d.y + 144 * S, { size: UI.FONT_BODY * S, color: P.shardL, weight: '800' });
    for (const eb of b.effects) {
      ...
      uiText(eb.name, eb.x + 10 * S, eb.y + 13 * S, { size: UI.FONT_BODY * S, color: eb.color, weight: '800' });
      this.clip1(eb.desc, eb.x + 52 * S, eb.y + 13 * S, eb.w - 130 * S, 9.5 * S, P.gray4);
      const labCol = state === 'owned' ? P.greenL : afford ? P.goldL : P.gray3;
      if (state === 'owned' || state === 'full') uiText(state === 'owned' ? '已鑲嵌' : '欄位已滿', eb.x + eb.w - 10 * S, eb.y + eb.h - 10 * S, { size: UI.FONT_BODY * S, align: 'right', color: labCol, weight: '800' });
      else goldLabel(eb.x + eb.w - 10 * S, eb.y + eb.h - 10 * S, forgeEffectCost(fdata.effects.length), { size: UI.FONT_BODY * S, align: 'right', color: labCol, weight: '800' });   // R17/2.1
    }
  ...
  drawFacilities(f) {
    ...
    for (const c of cards) {
      ...
      if (st === 'max' || st === 'gated') uiText(st === 'max' ? '已滿級' : '🔒 進度解鎖', c.x + c.w - 10 * S, c.y + c.h - 10 * S, { size: UI.FONT_BODY * S, align: 'right', color: col, weight: '800' });
      else goldLabel(c.x + c.w - 10 * S, c.y + c.h - 10 * S, this.hubCost(def.cost(cur), 'facilityPurchases'), { size: UI.FONT_BODY * S, align: 'right', color: col, weight: '800' });   // R17/2.1
    }
    ctx.restore();
    this.drawScrollbar(f);
  },
  ...
  drawQuests(f) {
    ...
    uiText('主線', f.x + 24 * S, t0 + 14 * S, { size: UI.FONT_BODY * S, color: P.goldL, weight: '800' });
    const cur = chapterState(META, META.questIndex || 0);
    if (cur) {
      uiText(cur.q.title, f.x + 24 * S, t0 + 30 * S, { size: UI.FONT_HEADING * S, color: '#fff', weight: '900' });
      uiText('目標：' + cur.q.desc, f.x + 24 * S, t0 + 46 * S, { size: UI.FONT_BODY * S, color: P.shardL, weight: '600' });
      ...
      if (can) goldLabel(L.mainClaim.x + L.mainClaim.w / 2, L.mainClaim.y + L.mainClaim.h / 2 + 1 * S, cur.q.reward, { size: UI.FONT_BODY * S, align: 'center', baseline: 'middle', color: '#fff', weight: '800', prefix: '領取 ' });   // R17/2.1
      else uiText('尚未達成', L.mainClaim.x + L.mainClaim.w / 2, L.mainClaim.y + L.mainClaim.h / 2 + 1 * S, { size: UI.FONT_BODY * S, align: 'center', baseline: 'middle', color: P.gray3, weight: '800' });
      ...
      uiText(trk ? '✓ 追蹤中' : '＋ 追蹤主線', L.mainTrack.x + L.mainTrack.w / 2, L.mainTrack.y + L.mainTrack.h / 2 + 1 * S, { size: UI.FONT_BODY * S, align: 'center', baseline: 'middle', color: trk ? P.shardL : '#fff', weight: '800' });
    } else uiText('主線已全數完成 — 你已成為魂晶之主。', f.x + 24 * S, t0 + 40 * S, { size: UI.FONT_BODY * S, color: P.goldL, weight: '800' });
    uiText('委託（本週懸賞 + 一般懸賞 · 可同時追蹤 ' + trackedCount(META) + '/' + MAX_TRACKED + '）', f.x + 24 * S, t0 + 118 * S, { size: UI.FONT_BODY * S, color: P.shardL, weight: '800' });
    ...
    uiText('📅 本週懸賞', f.x + 24 * S, L.wTop - 6 * S - (this.panelScroll || 0), { size: UI.FONT_BODY * S, color: P.goldL, weight: '800' });
    for (const r of L.weeklyRows) {
      ...
      uiText(r.q.title, f.x + 34 * S, r.y + 15 * S, { size: UI.FONT_BODY * S, color: st.claimed ? P.gray3 : P.holyL, weight: '800' });
      uiText(r.q.desc + '（' + Math.floor(st.prog) + '/' + r.q.goal + '）', f.x + 34 * S, r.y + 30 * S, { size: UI.FONT_CAPTION * S, color: st.done ? P.greenL : P.gray4 });
      ...
      if (st.claimed) uiText('✓ 已領', r.claim.x + r.claim.w / 2, r.claim.y + r.claim.h / 2 + 1 * S, { size: UI.FONT_CAPTION * S, align: 'center', baseline: 'middle', color: P.greenL, weight: '600' });
      else if (done) goldLabel(r.claim.x + r.claim.w / 2, r.claim.y + r.claim.h / 2 + 1 * S, r.q.reward, { size: UI.FONT_CAPTION * S, align: 'center', baseline: 'middle', color: '#fff', weight: '600', prefix: '領 ' });
      else uiText('進行中', r.claim.x + r.claim.w / 2, r.claim.y + r.claim.h / 2 + 1 * S, { size: UI.FONT_CAPTION * S, align: 'center', baseline: 'middle', color: P.gray3, weight: '600' });
    }
    uiText('一般懸賞', f.x + 24 * S, L.top - 12 * S - (this.panelScroll || 0), { size: UI.FONT_BODY * S, color: P.shardL, weight: '800' });
    if (!L.rows.length) uiText('目前沒有可接的委託（達成條件可解鎖隱藏委託）', f.x + 34 * S, L.top + 8 * S - (this.panelScroll || 0), { size: UI.FONT_BODY * S, color: P.gray3 });
    for (const r of L.rows) {
      ...
      uiText(q.title, f.x + 34 * S, r.y + 15 * S, { size: UI.FONT_BODY * S, color: lockedBy ? P.gray2 : (hidden ? P.purpleL : '#fff'), weight: '800' });
      if (lockedBy) {
        uiText('🔒 需先完成：' + lockedBy, f.x + 34 * S, r.y + 30 * S, { size: UI.FONT_CAPTION * S, color: P.gray3 });
      } else {
        uiText(q.desc + '（' + fmtQuestVal(p, q.fmt) + '/' + fmtQuestVal(q.goal, q.fmt) + '）', f.x + 34 * S, r.y + 30 * S, { size: UI.FONT_CAPTION * S, color: done ? P.greenL : P.gray4 });
        ...
        uiText(trk ? '✓ 追蹤中' : '＋ 追蹤', r.track.x + r.track.w / 2, r.track.y + r.track.h / 2 + 1 * S, { size: UI.FONT_CAPTION * S, align: 'center', baseline: 'middle', color: trk ? P.shardL : '#fff', weight: '600' });
        const cH = inside(mx, my, r.claim);
        ...
        if (done) goldLabel(r.claim.x + r.claim.w / 2, r.claim.y + r.claim.h / 2 + 1 * S, q.reward, { size: UI.FONT_CAPTION * S, align: 'center', baseline: 'middle', color: '#fff', weight: '600', prefix: '領 ' });   // R17/2.1
        else uiText('進行中', r.claim.x + r.claim.w / 2, r.claim.y + r.claim.h / 2 + 1 * S, { size: UI.FONT_CAPTION * S, align: 'center', baseline: 'middle', color: P.gray3, weight: '600' });
      }
    }
  ...
  drawGuildRank(f) {
    ...
    uiText('當前階級 · ' + gp.name, f.x + 24 * S, t0 + 18 * S, { size: UI.FONT_HEADING * S, color: P.gold, weight: '900' });
    uiText('聲望 ' + gp.xp + (gp.next ? '　·　距「' + gp.next.name + '」還需 ' + gp.toNext : '　·　已達頂階'), f.x + 24 * S, t0 + 38 * S, { size: UI.FONT_BODY * S, color: P.gray3 });
    ...
    for (const r of L.rows) {
      ...
      uiText(clampTo((reached ? '★ ' : '☆ ') + r.rk.name + '　·　需 ' + r.rk.xp + ' 聲望', 12 * S, '800'), txtL, r.y + 15 * S, { size: UI.FONT_BODY * S, color: reached ? '#fff' : P.gray3, weight: '800' });
      if (r.rk.reward) uiText(clampTo('獎勵：' + r.rk.reward.label, UI.FONT_CAPTION * S, '600'), txtL, r.y + 28 * S, { size: UI.FONT_CAPTION * S, color: claimed ? P.greenL : P.gray4 });
      if (r.rk.reward) {
        const cH = inside(mx, my, r.claim);
        ...
        uiText(claimed ? '✓ 已領取' : canClaim ? '領取獎勵' : '未達成', r.claim.x + r.claim.w / 2, r.claim.y + r.claim.h / 2 + 1 * S, { size: UI.FONT_CAPTION * S, align: 'center', baseline: 'middle', color: claimed ? P.greenL : canClaim ? '#fff' : P.gray3, weight: '800' });
      }
    }
    ctx.restore();
    this.drawScrollbar(f);
