...
export const renderMixin = {
  ...
  drawCoopTags() {
    ...
    for (const slot of this.coop.players) {
      ...
      uiText(slot.name + (isSelf ? '（你）' : ''), ns.x, ns.y, { size: UI.FONT_CAPTION * S, align: 'center', color: isSelf ? P.shardL : '#cfe0ff', weight: UI.WEIGHT_BODY, shadowColor: withAlpha('#000', 0.8) });
  ...
  drawShrine() {
    ...
    uiText('祝福神龕', ns.x, ns.y, { size: UI.FONT_BODY * S, align: 'center', color: P.shardL, weight: UI.WEIGHT_BODY });
    if (this.nearShrine) { const ps = worldToScreen(p.x, p.y + 8); uiText('按 E 祈福', ps.x, ps.y, { size: UI.FONT_BODY * S, align: 'center', color: withAlpha('#fff', 0.6 + Math.sin(this.t * 6) * 0.3), weight: UI.WEIGHT_BODY }); }
  },
  drawNpcs() {
    ...
    for (const n of this.npcs) {
      ...
      uiText(k.label, ns.x, ns.y, { size: UI.FONT_CAPTION * S, align: 'center', color: k.color, weight: UI.WEIGHT_BODY });
      if (this.nearNpc === n) { const ps = worldToScreen(n.x, n.y + 8); uiText('按 E', ps.x, ps.y, { size: UI.FONT_CAPTION * S, align: 'center', color: withAlpha('#fff', 0.6 + Math.sin(this.t * 6) * 0.3), weight: UI.WEIGHT_BODY }); }
    }
  },
  drawShopPanel() {
    ...
    uiText('魂 晶 商 店', L.x + 22 * S, L.y + 30 * S, { size: UI.FONT_TITLE * S, color: '#fff', weight: UI.WEIGHT_TITLE });
    const ssp = getSprite('shard'); drawSpriteUI(ssp.frames[0], L.x + L.w - 156 * S, L.y + 12 * S, 2 * S);
    uiText(String(this.run.shards), L.x + L.w - 132 * S, L.y + 30 * S, { size: UI.FONT_HEADING * S, color: P.shardL, weight: UI.WEIGHT_HEADING });
    uiRect(L.close.x, L.close.y, L.close.w, L.close.h, withAlpha('#3a2030', 0.9), { radius: 6 * S, stroke: P.redD, lw: 2 });
    uiText('✕', L.close.x + L.close.w / 2, L.close.y + L.close.h / 2 + 1 * S, { size: UI.FONT_BODY * S, align: 'center', baseline: 'middle', color: P.redL, weight: UI.WEIGHT_BODY });
    uiText('裝備鐵砧（三選一）', L.gearX, L.top - 14 * S, { size: UI.FONT_HEADING * S, color: P.goldL, weight: UI.WEIGHT_HEADING });
    uiText('能力值鐵砧（三選一）', L.anvilX, L.top - 14 * S, { size: UI.FONT_HEADING * S, color: P.shardL, weight: UI.WEIGHT_HEADING });
    uiText('鍛造鐵砧後三選一：史詩/稜彩裝備，或能力值強化', L.x + L.w / 2, L.y + 56 * S, { size: UI.FONT_CAPTION * S, align: 'center', color: P.gray3 });
    const buyCard = (c, title, sub, price, accent) => {
      ...
      uiText(title, c.x + 10 * S, c.y + 22 * S, { size: UI.FONT_BODY * S, color: '#fff', weight: UI.WEIGHT_BODY });
      uiText(sub, c.x + 10 * S, c.y + 40 * S, { size: UI.FONT_CAPTION * S, color: P.gray4 });
      uiText('魂晶 ' + price, c.x + c.w - 10 * S, c.y + c.h - 9 * S, { size: UI.FONT_CAPTION * S, align: 'right', color: afford ? accent : P.redL, weight: UI.WEIGHT_BODY });
    };
    ...
    uiText('點擊購買　·　B / Esc 關閉', L.x + L.w / 2, L.y + L.h - 13 * S, { size: UI.FONT_CAPTION * S, align: 'center', color: P.gray3 });
    if (this.shopFlashT > 0) { this.shopFlashT -= 1 / 60; uiText(this.shopFlash, L.x + L.w / 2, L.y + L.h - 30 * S, { size: UI.FONT_BODY * S, align: 'center', color: P.redL, weight: UI.WEIGHT_BODY }); }
    ...
    if (this.shopChoice && L.choiceCards) {
      ...
      uiText(gear ? '選擇一件裝備' : '選擇一項能力值強化', view.W / 2, L.choiceCards[0].y - 22 * S, { size: UI.FONT_HEADING * S, align: 'center', color: gear ? P.goldL : P.shardL, weight: UI.WEIGHT_HEADING });
      for (const c of L.choiceCards) {
        ...
        if (gear) {
          ...
          uiText(o.name + ' · ' + slotName, c.x + c.w / 2, c.y + 56 * S, { size: UI.FONT_BODY * S, align: 'center', color: rar, weight: UI.WEIGHT_BODY });
          const nLines = this.wrapText(o.desc || '', c.x + c.w / 2, c.y + 74 * S, c.w - 16 * S, UI.FONT_CAPTION * S, P.gray4);
          ...
          uiText('⚒', c.x + c.w / 2, c.y + 24 * S, { size: UI.FONT_TITLE * S, align: 'center', color: P.shardL, weight: UI.WEIGHT_TITLE });   // 4.18: stat-anvil emblem
          uiText(o.name, c.x + c.w / 2, c.y + 48 * S, { size: UI.FONT_BODY * S, align: 'center', color: '#fff', weight: UI.WEIGHT_BODY });
          this.wrapText(o.desc, c.x + c.w / 2, c.y + 68 * S, c.w - 16 * S, UI.FONT_BODY * S, P.emberL);
        }
      }
      ...
      uiText('跳過（放棄此次鍛造）', sk.x + sk.w / 2, sk.y + sk.h / 2 + 1 * S, { size: UI.FONT_BODY * S, align: 'center', baseline: 'middle', color: skh ? '#fff' : P.gray3, weight: UI.WEIGHT_BODY });
    }
  },
  drawShopCard(c, kind, mx, my, S) {
    ...
    if (kind === 'gear') {
      ...
      uiText(o.def.name, c.x + 38 * S, c.y + 18 * S, { size: UI.FONT_BODY * S, color: o.bought ? P.gray3 : rar, weight: UI.WEIGHT_BODY });
      this.clipShop(o.def.desc || '', c.x + 38 * S, c.y + 32 * S, c.w - 46 * S, UI.FONT_CAPTION * S);
      uiText(o.bought ? '已購買' : ('魂晶 ' + o.price), c.x + c.w - 8 * S, c.y + c.h - 7 * S, { size: UI.FONT_CAPTION * S, align: 'right', color: o.bought ? P.greenL : afford ? P.shardL : P.redL, weight: UI.WEIGHT_BODY });
    } else {
      ...
      uiText(a.name + (a.buys ? ' ×' + a.buys : ''), c.x + 10 * S, c.y + 18 * S, { size: UI.FONT_BODY * S, color: '#fff', weight: UI.WEIGHT_BODY });
      this.clipShop(a.desc, c.x + 10 * S, c.y + 32 * S, c.w - 18 * S, UI.FONT_CAPTION * S);
      uiText('魂晶 ' + price, c.x + c.w - 8 * S, c.y + c.h - 7 * S, { size: UI.FONT_CAPTION * S, align: 'right', color: afford ? P.shardL : P.redL, weight: UI.WEIGHT_BODY });
    }
  },
  clipShop(str, x, y, maxw, size) {
    let s = str; while (s.length > 1 && textWidth(s, size, UI.WEIGHT_BODY) > maxw) s = s.slice(0, -1);
    if (s.length < str.length && s.length > 1) s = s.slice(0, -1) + '…';
    uiText(s, x, y, { size, color: P.gray4, weight: UI.WEIGHT_BODY });
  },
  ...
  drawStageHud() {
    ...
    uiText(`${this.map.biome.name} · ${diffLabel} · 威脅 ${this.threat} · ⏱ ${em}:${es.toString().padStart(2, '0')}`, view.W / 2, 24 * S, { size: UI.FONT_HEADING * S, align: 'center', color: '#fff', weight: UI.WEIGHT_HEADING });
    let label, hot = false;
    ...
    uiText(label, view.W / 2, 42 * S, { size: UI.FONT_BODY * S, align: 'center', color: hot ? P.redL : P.gray3, weight: UI.WEIGHT_BODY });
    if (this.boss && this.bossRef && !this.bossRef.dead) {
      const bw = Math.min(360 * S, view.W * 0.5);
      uiText((this.reaperRef && this.bossRef === this.reaperRef ? '☠ ' : this.finalBoss ? '★ ' : '') + this.bossRef.def.name, view.W / 2, 56 * S, { size: UI.FONT_HEADING * S, align: 'center', color: P.redL, weight: UI.WEIGHT_HEADING });
      ...
      uiText('當前敵潮：' + this.activeTypes.map((d) => d.name).join('、'), view.W / 2, 54 * S, { size: UI.FONT_BODY * S, align: 'center', color: P.gray3 });
    }
  },
  ...
  drawInfo() {
    ...
    else uiText('Tab：build　·　B：商店　·　M：放大地圖' + (this.fusionReady ? '　·　✦ 可合成' : ''), view.W - 12 * S, view.H - 10 * S, { size: UI.FONT_CAPTION * S, align: 'right', color: withAlpha(this.fusionReady ? P.goldL : '#fff', this.fusionReady ? 0.7 : 0.28) });
  },
  ...
  drawWorldTip(info, mx, my, S) {
    ...
    uiText(info.name, x + 8 * S, y + 17 * S, { size: UI.FONT_BODY * S, color: '#fff', weight: UI.WEIGHT_BODY });
  ...
  drawBondBadge(x, y, sz, b, pg, S) {
    ...
    uiText(b.tag, x + sz / 2, y + sz / 2 + 0.5 * S, { size: UI.FONT_CAPTION * S, align: 'center', baseline: 'middle', color: achieved ? '#1a1404' : near ? '#dfe6ff' : P.gray3, weight: UI.WEIGHT_BODY });
    if (achieved) uiText(String(pg.level), x + sz - 1.5 * S, y + sz - 0.5 * S, { size: UI.FONT_CAPTION * S, align: 'right', color: '#2a1d00', weight: UI.WEIGHT_BODY, shadow: false });
  },
  ...
  drawBondTooltip(ic, mx, my, S) {
    ...
    uiText(b.tag, x + 17 * S, tagCY, { size: UI.FONT_BODY * S, align: 'center', baseline: 'middle', color: achieved ? '#1a1404' : P.gray3, weight: UI.WEIGHT_BODY });
    uiText(b.name, x + 32 * S, tagCY, { size: UI.FONT_HEADING * S, baseline: 'middle', color: '#fff', weight: UI.WEIGHT_HEADING });
    uiText('第 ' + pg.level + ' / ' + pg.max + ' 階', x + W - 8 * S, tagCY, { size: UI.FONT_CAPTION * S, align: 'right', baseline: 'middle', color: achieved ? P.goldL : P.gray3, weight: UI.WEIGHT_BODY });
    uiText('需求：' + b.goal, x + 8 * S, y + 36 * S, { size: UI.FONT_CAPTION * S, color: P.gray3, weight: UI.WEIGHT_BODY });
    b.tiers.forEach((t, k) => {
      const reached = pg.level >= k + 1, ty = y + 50 * S + k * 14 * S;
      uiText((reached ? '✓ ' : '· ') + '第 ' + (k + 1) + ' 階（達成 ' + t.at + '）', x + 8 * S, ty, { size: UI.FONT_CAPTION * S, color: reached ? '#dfe6ff' : P.gray3, weight: UI.WEIGHT_BODY });
      uiText(t.bonusDesc, x + W - 8 * S, ty, { size: UI.FONT_CAPTION * S, align: 'right', color: reached ? P.emberL : P.gray2, weight: UI.WEIGHT_BODY });
    });
  },
  drawTooltip(ic, mx, my, S) {
    ...
    uiText(def.name || ic.id || '?', x + 8 * S, y + 16 * S, { size: UI.FONT_HEADING * S, color: RARITY[rarityOf(def)].accent, weight: UI.WEIGHT_HEADING });   // R17/5.1: name reads its rarity
    uiText(sub, x + W - 8 * S, y + 16 * S, { size: UI.FONT_CAPTION * S, align: 'right', color: accent, weight: UI.WEIGHT_BODY });
    lines.forEach((l, i) => uiText(l, x + 8 * S, y + 32 * S + i * 14 * S, { size: UI.FONT_BODY * S, color: P.gray4, weight: UI.WEIGHT_BODY }));
    if (evo) uiText(evo.text, x + 8 * S, y + 32 * S + lines.length * 14 * S, { size: UI.FONT_CAPTION * S, color: evo.col, weight: UI.WEIGHT_BODY });
  },
  drawBuildPanel(S) {
    ...
    uiText('當前 BUILD', x + w / 2, y + 26 * S, { size: UI.FONT_TITLE * S, align: 'center', color: '#fff', weight: UI.WEIGHT_TITLE });
    uiText('Tab 關閉　·　滑鼠移到圖示看說明', x + w / 2, y + 44 * S, { size: UI.FONT_CAPTION * S, align: 'center', color: P.gray3 });
    this.buildIcons = [];
    const sz = 30 * S, gap = 6 * S;
    const head = (t, xx, yy, c, extra, ecol) => { uiText(t, xx, yy, { size: UI.FONT_HEADING * S, color: c, weight: UI.WEIGHT_HEADING }); if (extra) uiText(extra, xx + textWidth(t, UI.FONT_HEADING * S, UI.WEIGHT_HEADING) + 10 * S, yy, { size: UI.FONT_CAPTION * S, color: ecol || P.gray3, weight: UI.WEIGHT_BODY }); };
    const cell = (bx, by, sp, stroke, badge, bcol) => { uiRect(bx, by, sz, sz, withAlpha('#10121f', 0.82), { radius: 4 * S, stroke, lw: 2 }); drawSpriteUI(sp.frames[0], bx + 3 * S, by + 3 * S, (sz - 6 * S) / sp.w); if (badge) uiText(badge, bx + sz - 3 * S, by + sz - 3 * S, { size: UI.FONT_CAPTION * S, align: 'right', color: bcol, weight: UI.WEIGHT_BODY }); };
    ...
    [['weapon', '專武'], ['armor', '護甲'], ['trinket', '飾品']].forEach(([slot, label], i) => {
      ...
      else { uiRect(bx, yR, sz, sz, withAlpha('#10121f', 0.82), { radius: 4 * S, stroke: P.ink2, lw: 2 }); uiText('—', bx + sz / 2, yR + sz / 2 + 4 * S, { size: UI.FONT_BODY * S, align: 'center', color: P.gray2 }); }
      uiText(label, bx + sz / 2, yR + sz + 11 * S, { size: UI.FONT_CAPTION * S, align: 'center', color: P.gray3 });
    });
    yR += sz + 32 * S;
    ...
    const sFont = Math.max(UI.FONT_CAPTION * S, Math.min(UI.FONT_BODY * S, stride * 0.74));
    for (const [k, v] of stats) { if (yR > bandTop - 8 * S) break; uiText(k, colR, yR, { size: sFont, color: P.gray3, weight: UI.WEIGHT_BODY }); uiText(v, x + w - 24 * S, yR, { size: sFont, align: 'right', color: '#fff', weight: UI.WEIGHT_BODY }); yR += stride; }
    ...
    if (!achieved.length) { uiText('尚未觸發任何羈絆 — 湊齊特定武器／被動組合即可啟動', colL, bandTop + 38 * S, { size: UI.FONT_CAPTION * S, color: P.gray2, weight: UI.WEIGHT_BODY }); }
    else {
      ...
      achieved.forEach((o, i) => {
        ...
        while (nm.length > 1 && textWidth(nm, UI.FONT_CAPTION * S, UI.WEIGHT_BODY) > cellW - bsz - 10 * S) nm = nm.slice(0, -1);
        uiText(nm, cx2 + bsz + 5 * S, ry, { size: UI.FONT_CAPTION * S, color: P.goldL, weight: UI.WEIGHT_BODY });
  ...
  drawBanner() {
    ...
    uiText(this.banner, view.W / 2, Math.max(view.H * 0.2, 118 * S), { size: UI.FONT_TITLE * S, align: 'center', color: withAlpha('#ffe9a0', a), weight: UI.WEIGHT_TITLE, shadowColor: withAlpha('#000', a * 0.8) });
  },
  ...
  drawStory() {
    ...
    uiText(st.chapter ? ('第 ' + ((META.questIndex || 0) + 1) + ' 章') : (st.who || '角色'), view.W / 2, by + 22 * S, { size: UI.FONT_CAPTION * S, align: 'center', color: withAlpha(P.shardL, a), weight: UI.WEIGHT_BODY });
    uiText(st.title, view.W / 2, by + 44 * S, { size: UI.FONT_TITLE * S, align: 'center', color: withAlpha(P.goldL, a), weight: UI.WEIGHT_TITLE, shadowColor: withAlpha('#000', a) });
    const reveal = Math.floor(Math.min(st.text.length, (st.dur - st.t) / 0.03));   // typewriter reveal
    this.wrapText(st.text.slice(0, reveal), view.W / 2, by + 76 * S, view.W * 0.7, UI.FONT_BODY * S, withAlpha('#d8e0f0', a));
    if (st.quote) uiText('「' + st.quote + '」　— ' + (st.who || ''), view.W / 2, by + bandH - 34 * S, { size: UI.FONT_BODY * S, align: 'center', color: withAlpha(P.shardL, a * 0.95), weight: UI.WEIGHT_BODY });   // 角色劇情: signature battle quote
    uiText('按 空白鍵 跳過', view.W / 2, by + bandH - 14 * S, { size: UI.FONT_CAPTION * S, align: 'center', color: withAlpha('#fff', 0.4 * a) });
  },
};
