...
export const overlaysMixin = {
  ...
  drawCheatPanel() {
    const S = uiScale();
    if (Cheats.toast > 0) uiText('☠ 開發者模式 ' + (Cheats.enabled ? '開啟' : '關閉'), view.W / 2, view.H * 0.12, { size: UI.FONT_TITLE * S, align: 'center', color: P.goldL, weight: UI.WEIGHT_TITLE });
    if (!Cheats.enabled) return;
    const btns = this.cheatButtons();
    const mx = mouse.x * view.dpr, my = mouse.y * view.dpr;
    uiText('☠ DEV', btns[0].x + btns[0].w / 2, btns[0].y - 9 * S, { size: UI.FONT_CAPTION * S, align: 'center', color: P.redL, weight: UI.WEIGHT_BODY });
    for (const b of btns) {
      ...
      uiText(b.label, b.x + b.w / 2, b.y + b.h / 2 + 1 * S, { size: UI.FONT_BODY * S, align: 'center', baseline: 'middle', color: '#fff', weight: UI.WEIGHT_BODY });
    }
    const last = btns[btns.length - 1];
    uiText('※ 使用後本局不計排行榜', last.x + last.w / 2, last.y + last.h + 10 * S, { size: UI.FONT_CAPTION * S, align: 'center', color: P.redL, weight: UI.WEIGHT_BODY });
  },
  ...
  drawWon() {
    ...
    const cx = view.W / 2;
    uiText('關 卡 通 關！', cx, view.H * 0.11, { size: UI.FONT_TITLE * S, align: 'center', color: P.goldL, weight: UI.WEIGHT_TITLE });
    ...
    lines.forEach((l, i) => uiText(l, cx, view.H * 0.11 + (40 + i * 20) * S, { size: UI.FONT_BODY * S, align: 'center', color: i === 1 ? (this.reaperSlain ? P.goldL : P.gray3) : '#d8e8d0', weight: UI.WEIGHT_BODY }));
    if (this.run.bankRepaid > 0) uiText('🏦 銀行還款 -' + goldStr(this.run.bankRepaid) + (META.bank && META.bank.debt > 0 ? '（尚欠 ' + goldStr(META.bank.debt) + '）' : ''), cx, view.H * 0.11 + (40 + lines.length * 20) * S, { size: UI.FONT_BODY * S, align: 'center', color: P.emberL, weight: UI.WEIGHT_BODY });   // 7.2
    ...
    uiText('點擊 / 空白鍵 返回城鎮', cx, view.H * 0.95, { size: UI.FONT_CAPTION * S, align: 'center', color: withAlpha('#ffd479', 0.5 + blink * 0.5), weight: UI.WEIGHT_BODY });
  },
  ...
  drawDeath() {
    ...
    if (this.deathT < 0.3) return;
    const cx = view.W / 2;
    uiText('探 索 結 束', cx, view.H * 0.11, { size: UI.FONT_TITLE * S, align: 'center', color: P.redL, weight: UI.WEIGHT_TITLE });
    ...
    lines.forEach((l, i) => uiText(l, cx, view.H * 0.11 + (40 + i * 20) * S, { size: UI.FONT_BODY * S, align: 'center', color: i === 1 ? P.goldL : '#d8def0', weight: UI.WEIGHT_BODY }));
    if (this.run.bankRepaid > 0) uiText('🏦 銀行還款 -' + goldStr(this.run.bankRepaid) + (META.bank && META.bank.debt > 0 ? '（尚欠 ' + goldStr(META.bank.debt) + '）' : ''), cx, view.H * 0.11 + (40 + lines.length * 20) * S, { size: UI.FONT_BODY * S, align: 'center', color: P.emberL, weight: UI.WEIGHT_BODY });   // 7.2
  ...
  drawResultSummary(topY) {
    ...
    uiText('本局配置', colL, yL, { size: UI.FONT_HEADING * S, color: P.shardL, weight: UI.WEIGHT_HEADING });
    uiText('滑鼠移到圖示看效果', colL + 86 * S, yL, { size: UI.FONT_CAPTION * S, color: P.gray3 }); yL += SEC_GAP;
    uiText('武器', colL, yL, { size: UI.FONT_HEADING * S, color: P.gray3, weight: UI.WEIGHT_HEADING }); yL += HEAD_DROP;
    ...
    uiText('被動 ×' + abils.length, colL, yL, { size: UI.FONT_HEADING * S, color: P.manaL, weight: UI.WEIGHT_HEADING }); yL += HEAD_DROP;
    ...
    uiText('裝備', colL, yL, { size: UI.FONT_HEADING * S, color: P.goldL, weight: UI.WEIGHT_HEADING }); yL += HEAD_DROP;   // R17/1.6: own row, same rhythm as the others
    [['weapon'], ['armor'], ['trinket']].forEach(([slot], i) => { const bx = colL + i * (sz + gap); const d = eq[slot] && Equipment.get(eq[slot]); if (d) { cell(bx, yL, getSprite(iconOr(d.icon, 'equip_leather_armor')), P.goldL, '', ''); this.resultIcons.push({ x: bx, y: yL, w: sz, h: sz, kind: 'equip', def: d }); } else { uiRect(bx, yL, sz, sz, withAlpha('#10121f', 0.82), { radius: 4 * S, stroke: P.ink2, lw: 1 }); uiText('—', bx + sz / 2, yL + sz / 2 + 4 * S, { size: UI.FONT_BODY * S, align: 'center', color: P.gray2 }); } });
    yL += sz + SEC_GAP;
    ...
    if (bonds.length && yL + HEAD_DROP + 20 * S <= y + h - 50 * S) {
      uiText('羈絆', colL, yL, { size: UI.FONT_HEADING * S, color: P.goldL, weight: UI.WEIGHT_HEADING }); yL += HEAD_DROP;
    ...
    uiText('傷害排行', colR, yR, { size: UI.FONT_HEADING * S, color: P.emberL, weight: UI.WEIGHT_HEADING });
    uiText('來源 · 佔比', x + w - 18 * S, yR, { size: UI.FONT_CAPTION * S, align: 'right', color: P.gray3 }); yR += 18 * S;
    ...
    if (!dmgEntries.length) uiText('（本局無傷害紀錄）', colR, yR, { size: UI.FONT_BODY * S, color: P.gray3 });
    dmgEntries.slice(0, 6).forEach(([name, v], i) => {
      ...
      uiText((i + 1) + '. ' + name, colR + 5 * S, yR + 11 * S, { size: UI.FONT_BODY * S, color: '#fff', weight: UI.WEIGHT_BODY });
      uiText(fmtDmg(v) + ' · ' + pct + '%', colR + rw - 5 * S, yR + 11 * S, { size: UI.FONT_BODY * S, align: 'right', color: rankCol, weight: UI.WEIGHT_BODY });
      yR += 17 * S;
    });
    ...
    if (coach.length) {
      ...
      let yC = yR + 10 * S;
      if (yC + 15 * S <= yLimit) {
        uiText('⚑ 教練筆記', colR, yC, { size: UI.FONT_HEADING * S, color: P.shardL, weight: UI.WEIGHT_HEADING }); yC += 16 * S;
        const toneCol = { good: P.greenL, warn: P.redL, info: P.gray3 };
        for (const ln of coach) {
          if (yC > yLimit) break;
          const full = (ln.icon ? ln.icon + ' ' : '') + ln.text;
          let s = full; while (s.length > 1 && textWidth(s, UI.FONT_BODY * S, UI.WEIGHT_BODY) > rw) s = s.slice(0, -1);
          if (s.length < full.length && s.length > 1) s = s.slice(0, -1) + '…';
          uiText(s, colR, yC, { size: UI.FONT_BODY * S, color: toneCol[ln.tone] || '#cfd6ee', weight: UI.WEIGHT_BODY });
          yC += 15 * S;
        }
      }
    }
    ...
    let oy = y + h - 40 * S;
    uiText('★ 本局解鎖', colL, oy, { size: UI.FONT_HEADING * S, color: P.goldL, weight: UI.WEIGHT_HEADING }); oy += 15 * S;
    const items = [];
    for (const ac of un) items.push('成就「' + (ac.realName || ac.name) + '」' + (ac.rewardLabel ? ' → ' + ac.rewardLabel : ''));
    for (const c of nc) items.push('角色「' + c.name + '」');
    if (!items.length) uiText('（本局沒有新解鎖）', colL + 4 * S, oy, { size: UI.FONT_BODY * S, color: P.gray3 });
    else { items.slice(0, 2).forEach((t, i) => this.clipShop(t, colL + 4 * S, oy + i * 13 * S, w - 40 * S, UI.FONT_BODY * S)); if (items.length > 2) uiText('…等 ' + items.length + ' 項', x + w - 18 * S, oy, { size: UI.FONT_CAPTION * S, align: 'right', color: P.gray3 }); }
  ...
  drawCardBonds(r, oy, S, hints) {
    ...
    if (r.h < 190 * S) {   // tiny card: one-line compact hint, avoids overlapping the name
      ...
      while (s.length > 2 && textWidth(s, UI.FONT_CAPTION * S, UI.WEIGHT_BODY) > r.w - 18 * S) s = s.slice(0, -1);
      uiText(s, r.x + r.w / 2, by + 8 * S, { size: UI.FONT_CAPTION * S, align: 'center', baseline: 'middle', color: P.goldL, weight: UI.WEIGHT_BODY });
      return;
    }
    ...
    let y = by + 12 * S;
    uiText('★ ' + b.name, r.x + 12 * S, y, { size: UI.FONT_HEADING * S, color: P.goldL, weight: UI.WEIGHT_HEADING });
    uiText('第 ' + h.toLevel + ' / ' + h.max + ' 階', r.x + r.w - 12 * S, y, { size: UI.FONT_CAPTION * S, align: 'right', color: P.gold, weight: UI.WEIGHT_BODY });
    y += 14 * S;
    if (pg.parts && pg.parts.length) {                 // 組合型：逐件 ✓ / ▶(此選項填上) / ·
      ...
      for (const p of pg.parts) {
        ...
        const tw = textWidth(txt, UI.FONT_CAPTION * S, UI.WEIGHT_BODY);
        if (px + tw > r.x + r.w - 12 * S) { uiText('…', px, y, { size: UI.FONT_CAPTION * S, color: P.gray3, weight: UI.WEIGHT_BODY }); break; }
        uiText(txt, px, y, { size: UI.FONT_CAPTION * S, color: fill ? P.goldL : (p.ok ? P.greenL : P.gray2), weight: UI.WEIGHT_BODY });
        px += tw + 7 * S;
      }
    } else {                                            // 數量型：進度 X → X+1（下一階需 Y）
      const next = pg.nextTier ? '（下一階需 ' + pg.nextTier.at + '）' : '（已滿階）';
      uiText('進度 ' + pg.count + ' → ' + (pg.count + 1) + ' ' + next, r.x + 12 * S, y, { size: UI.FONT_CAPTION * S, color: P.gray4, weight: UI.WEIGHT_BODY });
    }
    y += 13 * S;
    let eff = (h.crosses ? '解鎖 ' : '推進 ') + (h.toward ? h.toward.bonusDesc : '');
    while (eff.length > 2 && textWidth(eff, UI.FONT_CAPTION * S, UI.WEIGHT_BODY) > innerW) eff = eff.slice(0, -1);
    uiText(eff, r.x + 12 * S, y, { size: UI.FONT_CAPTION * S, color: h.crosses ? P.emberL : P.shardL, weight: UI.WEIGHT_BODY });
    y += 13 * S;
    if (hints.length > 1) uiText('＋ 另推進 ' + (hints.length - 1) + ' 個羈絆', r.x + r.w / 2, y, { size: UI.FONT_CAPTION * S, align: 'center', color: P.gray3, weight: UI.WEIGHT_BODY });
  },
  ...
  drawChoicePeekBuild() {
    ...
    uiText('TAB 返回選擇強化', view.W / 2, 16 * S, { size: UI.FONT_BODY * S, align: 'center', color: P.goldL, weight: UI.WEIGHT_BODY });
  },
  drawChoice() {
    const S = uiScale();
    ...
    uiText('選 擇 強 化', view.W / 2, rects[0].y - 28 * S, { size: UI.FONT_TITLE * S, align: 'center', color: P.manaL, weight: UI.WEIGHT_TITLE });
    let hintLine = '點擊卡片或按 1 / 2 / 3　·　★ 金框＝可推進羈絆　·　TAB 查看 build';
    if (evoFlags.some((v) => v)) hintLine += '　·　◆＝已知進化路線';
    uiText(hintLine, view.W / 2, rects[0].y - 8 * S, { size: UI.FONT_CAPTION * S, align: 'center', color: P.gray3 });
    rects.forEach((r, i) => {
      ...
      const pw = textWidth(st.tag, UI.FONT_CAPTION * S, UI.WEIGHT_BODY) + 14 * S;              // rarity pill
      uiRect(r.x + r.w - pw - 8 * S, r.y + oy + 10 * S, pw, 16 * S, withAlpha(tc, 0.22), { radius: 8 * S, stroke: tc, lw: 1 });
      uiText(st.tag, r.x + r.w - pw / 2 - 8 * S, r.y + oy + 18 * S, { size: UI.FONT_CAPTION * S, align: 'center', baseline: 'middle', color: tc, weight: UI.WEIGHT_BODY });
      if (hints.length) uiText('★', r.x + r.w / 2, r.y + oy + 14 * S, { size: UI.FONT_BODY * S, align: 'center', baseline: 'middle', color: P.goldL, weight: UI.WEIGHT_BODY });
      if (evoFlags[i]) {   // P1 圖鑑：已發現進化配方 pill，疊在 rarity pill 正下方避免重疊
        const ew = textWidth('◆ 已知進化', UI.FONT_CAPTION * S, UI.WEIGHT_BODY) + 12 * S;
        uiRect(r.x + r.w - ew - 8 * S, r.y + oy + 30 * S, ew, 14 * S, withAlpha(P.astral, 0.22), { radius: 7 * S, stroke: P.astral, lw: 1 });
        uiText('◆ 已知進化', r.x + r.w - ew / 2 - 8 * S, r.y + oy + 37 * S, { size: UI.FONT_CAPTION * S, align: 'center', baseline: 'middle', color: P.astralL, weight: UI.WEIGHT_BODY });
      }
      ...
      uiText(st.sub, r.x + r.w / 2, midY + 12 * S, { size: UI.FONT_CAPTION * S, align: 'center', color: st.accent, weight: UI.WEIGHT_BODY });
      uiText(c.def.name, r.x + r.w / 2, midY + 31 * S, { size: UI.FONT_HEADING * S, align: 'center', color: '#fff', weight: UI.WEIGHT_HEADING });
      let dy = midY + 49 * S;
      if (st.effect) { const n = this.wrapText(st.effect, r.x + r.w / 2, dy, r.w - 22 * S, UI.FONT_BODY * S, P.emberL); dy += n * (12 * S + 3) + 5 * S; }
      this.wrapText(st.desc || '', r.x + r.w / 2, dy, r.w - 22 * S, UI.FONT_BODY * S, P.gray4);
      ...
      const tpw = textWidth(tlab, UI.FONT_CAPTION * S, UI.WEIGHT_BODY) + 14 * S;
      uiRect(r.x + 8 * S, r.y + oy + 10 * S, tpw, 16 * S, withAlpha(ti.col, 0.2), { radius: 8 * S, stroke: ti.col, lw: 1 });
      uiText(tlab, r.x + 8 * S + tpw / 2, r.y + oy + 18 * S, { size: UI.FONT_CAPTION * S, align: 'center', baseline: 'middle', color: ti.col, weight: UI.WEIGHT_BODY });
