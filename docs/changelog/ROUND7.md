...
## 連線實測回報的三項修正（round 7.1）

實機多人連線測試後修掉使用者回報的三個問題：

1. **斷線的玩家仍顯示在遊戲中** → 不再只靠 WS 心跳（原 30s）。主機端對每個遠端 avatar 加**輸入靜默計時**（連線中的訪客即使站著不動也以 ~30Hz 送輸入,>4 秒無輸入即判定離線),搭配 clean-close 的 `peer:left` 立即退場;退場後重檢遊戲結束條件(全員陣亡才結束,主機還活著則續局)。WS 心跳也由 30s→12s。訪客端也加快照靜默偵測(>6 秒無快照→視為房主離線)。離線的 avatar 立刻帶上 `dead` 旗標,所以所有人的畫面都會把他移除。離線測試:`__DBG.coopSilenceTest()` → `leftAfterSilence:true, runEnded:false`。
2. **多人升等不跳升級選單** → 改成**非阻塞**的升級選單(共享世界無法暫停,所以世界照跑、選單浮在下方,可點擊或按 1/2/3,逾時自動選)。主機自己拿完整的單機式三選一(`coopPick`);每位訪客拿主機算好的**武器三選一**(`levelup` 訊息),選擇透過 `levelpick` 回傳主機套用到該訪客的 avatar。實測:主機選單會開、訪客收到 6 次升級選單、選擇生效(該訪客武器 1→5 把)。
3. **許多 UI 提示在多人下未同步** → 主機的中央橫幅(敵潮更替、小王現身、蘑菇地雷、羈絆、升級、通關…)與階段資訊(距小王倒數/通關狀態)現在隨快照 `hud` 一起送給訪客顯示。實測:訪客同步看到 8 種不同橫幅。

伺服器另加 `levelup`/`levelpick` 兩種中繼訊息(納入 `game` 限流類別)。修正後伺服器測試仍 33 項全綠,單機與合作往返自測、真機端到端皆通過。
