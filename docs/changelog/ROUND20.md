...
## R20.1 — 主畫面封面改版（含遊戲內更新日誌 V 版化）

**遊戲內更新日誌**（前一筆 commit）：`patchnotes.js` 改語意化版本（V1.0=R18／V1.5=R19／V2.0=R20，`GAME_VERSION='V2.0'`）；條目支援 `{h,t}` 分類標籤格式；`title.js` 新增 `noteSegs/drawRich`——`《系統名》` 染魂青、數值 token 自動金色高亮（舊純字串條目也吃同一管線）。

**封面場景**（玩家需求：末日風＋多位英雄＋對面敵人高塔＋圍攻感）：
- 新美術 `src/art/title_scene.js`：**`title_tower`** 72×150 三幀魔眼黑塔（黑曜石塔身、餘燼窗、魂青裂紋、雙側殘塔、冠頂魔眼三幀脈動、尖塔信標、環繞符文碎片、左冷右暖 rim light）。
- `title.js drawBackdrop()`：全程序化、零狀態（皆由 `this.t` 推導）——暮紫天空＋右側餘燼輻射、34 顆閃爍星、**碎裂之月**（隕坑＋大裂縫＋脫離的碎塊）、遠景廢墟天際線、燃燒地平線光帶、三層山脊剪影、魂裂隙輝線、高塔（56% 視高）＋冠頂紅暈＋3 隻盤旋蝙蝠、中景廢墟剪影（拱門/枯樹/斷柱/戰旗，重用 town_ruin_* sprite）、**6 人英雄小隊**於左側山脊面向高塔（聖盾騎士/烈焰/玩家造型隊長(帶魂光)/星喚/守望者/劍舞者，錯落站位＋待機浮動＋影子）、**圍攻特效**（3 發魂青彈幕弧線飛向塔冠＋命中衝擊環、高塔紅彈反擊）、26 顆餘燼/灰燼飄浮、底部漸層暗幕保 UI 可讀。
- `drawLogo()`：LOGO 重製——魂光暈底、粗影＋墨描邊、**冰青→星紫漸層字**、雙 kira 閃光、金色分隔線＋菱形飾、字距化英文副標；slots 頁沿用緊湊版位置。
- **UI 配合**：選單按鈕換暮色主題（單人=冷灰藍+魂青框、多人=焦琥珀+金框）、hover 外圈光暈＋頂部玻璃高光線、更新日誌鈕改暗紫底；移除舊的中央漂浮英雄（小隊取代）。佈局/點擊範圍完全不變。

**驗證**：reload 零錯誤；選單/存檔頁/更新日誌三模式截圖（高塔與小隊局部放大圖逐項檢查）。

**玩家二修（同批）**：
1. **選單直排**：五鈕（單人/多人/排行榜/帳號/設定）改為 LOGO 下方**置中垂直一列**（`menuTop()` 與 LOGO 保持呼吸距離、`menuScale` 重新封頂防溢出、更新日誌鈕錨在列底），**配色統一**為冷灰藍玻璃＋魂青框（多人不再獨用金色）、**五鈕文字統一 16S/800**（三修：原大鈕 18/小鈕 13.5）；畫面下半讓給封面場景、底部暗幕減淡。
2. **高塔 Boss 駐軍**（三修共 4 隻，全為 R18 最新終 Boss 設計）：`b3_seraphjudge` 墮天審判懸浮於塔冠左側（浮動 bob）、`b3_sandpharaoh` 流沙法老鎮守塔門前、`b3_leviathan` 深淵利維坦盤踞塔基、`b3_bogmaw` 腐沼巨蟾母潛伏右翼（落地者含地面影）。
3. **小隊擴編 9 人**（以最新英雄為主）：後排施法者 4（傀儡師/時詠/虛空喚靈/守墓）小尺寸高位、中排 3（聖盾/星喚/龍槍）、前排先鋒 2（玩家造型隊長＋劍舞者）大尺寸低位——遠近大小分明；新增**前哨營地**（營火＋暖光暈、補給箱桶、戰旗×2、大門柱＋圍欄線/瓦礫**蓋在英雄前**做前景深度）；彈幕改為**角色武器 sprite**（隊長魂彈 bolt／聖騎聖彈 bolt_volt／虛空喚靈虛空珠 bolt_void，週期錯開、同時最多 3 發＋塔方紅彈降頻）。
4. **光影**：月亮加明暗界線（剪裁於月面）、亮側邊緣光、兩道斜落月光束；三層山脊稜線加「左月光青→右餘燼橙」漸層 rim 光；地面加雙光池（小隊腳下冷月光、塔基暖餘燼）。
