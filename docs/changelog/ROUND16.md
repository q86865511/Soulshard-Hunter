...
- **1.1 全域字型改標楷體**：`engine/renderer.js` 的 `FONT` 改為 KaiTi 優先鏈（`標楷體 / DFKai-SB / BiauKai / Kaiti TC / KaiTi`），原無襯線鏈保留為 fallback（無 KaiTi 的系統仍正常顯示）。`textWidth()` 與 `uiText()` 共用同一 `FONT`，故截斷/換行計算與實際渲染一致、不破版。
