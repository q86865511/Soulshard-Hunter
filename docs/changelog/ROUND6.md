...
## 6. 全面測試 + 架構稽核（Round 6.3）

第二輪 **Workflow `soulshard-qa-full`：77 個代理、~3.73M tokens、915 次工具呼叫**——14 個領域（引擎核心/音樂/世界戰鬥/玩家狀態/跑圖/大廳/地圖生態/武器技能/進度存檔/meta 系統/前端網路/後端伺服器/平衡/**整體架構**）各一名稽核員 → 對抗式逐條驗證 → 綜整報告。**62 項發現、60 項確認**（8 高、22 中、30 低）。

**總評：** 在正常路徑上**穩定**、資料驅動架構**健康**；但有一處結構脆弱（遊戲迴圈無容錯）＋ 新後端三項部署安全項。已修高優先 + 多數中優先：

| 區 | 修正 |
| --- | --- |
| 引擎(高) | **遊戲迴圈無容錯**：任一未捕捉例外會永久凍結遊戲 → `loop.js frame()` 包 try/catch、重置時鐘、無條件重排 rAF（一幀小故障自我復原） |
| 音樂(高) | 通關/死亡曲（不循環）播完後**永久靜音** → 加 `ended` 事件交還合成器後備 |
| 戰鬥(高) | 接觸/遠程**命中狀態（緩速/中毒）無視無敵幀/衝刺/閃避** → `takeDamage` 回傳是否命中，狀態只在真命中時施加 |
| 伺服器(高) | 弱 JWT 守衛只在 `NODE_ENV=production` 生效（可繞過）→ 改成**永遠 fail-closed**（弱/預設 secret 拒開機，`ALLOW_INSECURE_JWT=1` 才放行）；密碼上限改 **72 byte**（非字元）；Fastify `trustProxy:'loopback'` 讓反代後 rate-limit 以**真實 IP** 計 |
| 架構(高) | `integrate.mjs` 重整合會剝掉手加 import → header 補上 `registerHeroBody/drawHeroBody`（英雄包不再消失） |
| 平衡(中) | 死神接觸近乎一擊（重複套 `BOSS_DMG_MULT`）→ 死神 `dmgMult:1.0` + 調低 `REAPER_DMG_*`；新增 `FIRE_RATE_CAP 2.4`（射速倍率封頂）；玩家緩速縮短（不再被怪潮永久釘住）；DoT 無視無敵 → 加無敵幀守衛 |
| 跑圖(中) | 牆體生成可能封死孤立區（撿寶/怪卡死）→ 生成後**洪水填充連通性檢查**，封回不可達地板（實測 0 不可達）；魂牢突圍門檻夾到實際生成數（不會無法突圍）；特殊事件套用開局緩衝；棄局 bank 加防重複守衛 |
| 雲端(中) | 跨裝置存檔用 `Date.now()` 比較（時鐘可回退）→ 改用**單調 `saveSeq`** 計數器（前後端一致）；過 72-byte 密碼、過期 token、登入時序皆已處理；登入避免在局內覆蓋進度 |
| 經濟/裝備(中) | 簽名/裝備位武器**漏掉全域減傷** → 補 `PLAYER_DAMAGE_MULT`+暴擊上限；`shardMult` 對單顆掉落改**隨機進位**（碎片收入不再被四捨五入抹掉） |
| 排行榜(中) | 同一玩家可洗版 → 查詢改 **每人最佳一筆**（`DISTINCT ON`） |
| 雜項(低) | 公會頁支援滾輪、衣帽店付費重抽不再重置免費計時器、追蹤彈忽略生成無敵怪、`SURROUND_PERIOD`/`SAVE_VERSION` 補上單一真相來源/遷移接縫… |

> **驗證：** 後端 `inject()` **18/18**；前端 3000-tick 實戰模擬（真實受傷路徑）**零例外**、地圖連通性**0 不可達**、啟動乾淨。

### 6-4. 補完先前刻意保留的兩項
- **空間分割（P8，效能）：** `World` 新增**均勻網格**（64px cell，每幀 `rebuildGrid()`）＋ `forEachNear(x,y,r,cb)`；敵人**分離**（原 O(n²)，260 怪約 6.7 萬次/幀）、`nearestEnemy`/`aimTarget`、`dealAreaDamage`、`resolveCombat` 四處熱查詢全改走網格 → **O(n·k)**。實測 **220 怪零例外**、分離正常（怪群不疊）、戰鬥/瞄準/AoE 經網格仍正確。
- **鍛造「疾速」吃進更新型武器（P3）：** `player.updateWeapons` 把封頂後的射速倍率存到 `inst.fmHaste`；雷射/光環/砲塔/環繞等**自我計時**武器的 tick 與「每怪命中冷卻」改除以 `inst.fmHaste` → 疾速現在會加速它們（並受 `FIRE_RATE_CAP 2.4` 封頂）。實測：鍛造疾速 3× → `fmHaste` 由 0.82 升到 **2.4（封頂）**。
