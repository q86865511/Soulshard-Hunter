...
| 4.21 | **守護怪 + 鑰匙掉落視覺強化（新）** | `run.js`、`enemy.js`、`pickup.js`、`hud.js`、minimap |
| 4.22 | **持有鑰匙道具欄顯示 + 大地圖（M 鍵）守護怪／寶箱標示（新）** | `hud.js`、`run.js` bigMap、`world.js` |
...
| 8.4 | **魂晶使用途徑多樣化（新）** | `run.js`、`world.js`、`player.js`、`balance.js`、`hud.js` |
...
## 1.6 全局 UI 尺寸一致化（選單→子面板字型與元件比例統一）（新增）

### 問題

各個 UI 面板使用不同的字型大小與元件尺寸，造成「選單選項很大、點進去後子面板很小」的割裂感：

- 主選單（標題畫面 / Hub ESC 選單）按鈕：字體 18–22px，行高充裕
- Hub 面板（天賦/設施/鍛造/公會/衣帽間）：字體 11–13px，密集卡片
- 局內三選一升級卡：字體 14px，但間距不一致
- HUD 數值：11px 細字，與選單差距明顯

玩家在選單和子面板間切換時，眼睛需要重新「校準」，體驗割裂。

---

### 1.6-A 建立 UI 尺寸代幣系統（`engine/renderer.js`）

新增全局 `UI` 常數物件，所有 Canvas 繪製使用代幣而非魔法數字：

```js
// engine/renderer.js — 導出供全域使用
export const UI = {
  // 字型大小（乘以 S 後使用）
  FONT_TITLE:   22,   // 面板主標題、選單大按鈕
  FONT_HEADING: 16,   // 分區標題、Tab 標籤
  FONT_BODY:    13,   // 選項正文、說明
  FONT_CAPTION: 11,   // 標注、tooltip、計數

  // 元件尺寸
  BTN_H:        36,   // 按鈕高度
  BTN_PAD_X:    14,   // 按鈕橫向 padding
  CARD_H_MIN:   56,   // 卡片最小高度（升級/裝備選項）
  ICON_SM:      16,   // 小圖示（HUD 狀態）
  ICON_MD:      24,   // 中圖示（選項卡片）
  ICON_LG:      32,   // 大圖示（道具欄、升級主圖示）

  // 間距
  GAP_SM:        6,   // 緊湊間距（同組元素內）
  GAP_MD:       12,   // 標準間距（不同組之間）
  GAP_LG:       20,   // 大間距（面板分區）
...
**原則：** 相同層級的資訊在所有畫面使用相同代幣。「進入子面板」不代表字要變小，而是**密度增加**（更多資訊並排），但個別元素的字體大小保持相同層級。

| 元素 | 目前問題 | 統一後 |
|---|---|---|
| 面板主標題 | 部分 16px，部分 20px | 一律 `FONT_TITLE × S` = 22px |
| 分區標題（天賦分類、鍛造欄位名）| 11–14px 不一 | 一律 `FONT_HEADING × S` = 16px |
| 選項正文（天賦說明、效果文字）| 10–13px | 一律 `FONT_BODY × S` = 13px |
| tooltip / 小標注 | 9–11px | 一律 `FONT_CAPTION × S` = 11px |
| 主選單大按鈕 | 18–22px（偏大）| `FONT_TITLE × S` — 與面板標題同級，視覺一致 |
| Hub 面板按鈕 | 12–13px（偏小）| `FONT_BODY × S`，但 `BTN_H × S` 保持高度一致 |
| 升級卡片 | 14px 有時 12px | 標題 `FONT_HEADING`，說明 `FONT_BODY` |
| HUD 數值（HP / 金幣）| 11px | 維持 `FONT_CAPTION × S`，符合「輔助資訊」層級 |

---

### 1.6-C 重點修改位置

**標題畫面 (`scenes/title.js`)：**
- 「單人遊戲」/ 「多人連線」大按鈕：`FONT_TITLE`，`BTN_H × 1.5`（允許標題畫面稍大，但不超過 1.5×）
- 「排行榜」/「帳號」/「設定」次要按鈕：`FONT_BODY`，`BTN_H`

**Hub ESC 選單 (`scenes/hub.js`)：**
- 選單列表項目：`FONT_HEADING`（目前可能 18–20px），保持選項可讀但不過大
- 選中後**子面板**標題同樣是 `FONT_HEADING`，選項說明是 `FONT_BODY` ← 這樣不會有「進去後變小」的感覺
...
| 面板頂部大標題 | `FONT_TITLE` |
| Tab 標籤 | `FONT_HEADING` |
| 分類標籤（天賦分區名）| `FONT_HEADING` |
| 升級項目名稱 | `FONT_BODY` |
| 升級效果說明 | `FONT_CAPTION`（次要資訊可以稍小）|
| 費用金幣數字 | `FONT_BODY`（重要 → 不可過小）|
| 按鈕文字（確認/重置）| `FONT_BODY` |

**局內面板 (`run.js`)：**
- 升級三選一卡片：標題 `FONT_HEADING`，效果說明 `FONT_BODY`，稀有度標籤 `FONT_CAPTION`
- anvilChoice 同上
- 結算頁大標題：`FONT_TITLE`；各欄位標題：`FONT_HEADING`；數值：`FONT_BODY`

---

### 1.6-D 按鈕高度統一

**問題：** 選單按鈕高 `50px`，Hub 面板按鈕高 `24px`，高度差距讓點擊目標大小不一致。

**統一規則：**
- 主要行動按鈕（確認 / 購買 / 出擊）：`BTN_H × S` = 36px
- 次要按鈕（取消 / 重置 / 返回）：`BTN_H × 0.75 × S` ≈ 27px
- 選單列表項目（ESC 選單）：`BTN_H × 1.2 × S` ≈ 43px（選單允許稍大，提升點擊舒適度）
- Tab 標籤：固定 `28 × S`

**間距：** 所有面板內的按鈕列表使用 `GAP_SM` 分隔（6px），按鈕與面板邊緣留 `GAP_MD`（12px）。

---

### 1.6-E 漸進套用策略

為避免一次大改所有面板造成難以 debug，建議按以下順序套用：

1. 先在 `renderer.js` 定義 `UI` 常數並 export
2. 標題畫面 (`title.js`) — 最外層選單先對齊
3. Hub ESC 選單 → Hub 各面板（視覺跳躍問題的主要源頭）
4. 局內三選一、結算頁
5. HUD 數值（最後，影響範圍最廣）

每步驟完成後截圖對比，確認「選單→子面板視覺落差縮小」。

---

**驗證：**
- 標題畫面大按鈕字體 = Hub 面板主標題字體（同 `FONT_TITLE × S`）
- Hub ESC 選單選項字體 = Hub 面板分區標題（同 `FONT_HEADING × S`）
...
## 4.21 守護怪 + 鑰匙掉落視覺強化（新增）

**背景（Round 12）：** 房間守護怪（room guardian）擊殺後掉落 🔑 鑰匙，用來開鎖定金庫寶箱（`world.keys`）。目前守護怪和掉落的鑰匙外觀不夠醒目，玩家常常不知道「這隻怪是特別的」或「鑰匙掉在哪裡」。

---

### 4.21-A 守護怪外觀標識

**目標：** 玩家遠看就能一眼認出守護怪，並知道「擊殺它有特殊獎勵」。

**改動（`enemy.js` `draw()` + `run.js` 渲染）：**

1. **金色鑰匙懸浮圖示**：守護怪頭頂上方 `12px * S` 處，持續顯示小型金色 🔑 圖示（`16×16 * S`），緩慢上下浮動（振幅 `3px * S`，週期 `2s`），不受怪物面向影響。
2. **金色光暈邊框**：守護怪的 `outline()` 替換為雙層邊框——內層白色（原有），外層金色 `#f5c518`（`outline` 再呼叫一次偏移 `+1px`）+ 淡金色 `glow`（`engine/sprites.js glow` helper，已有）。
3. **怪物名稱標籤**：守護怪在 HP 條上方顯示「🔑 守護者」灰底白字小標籤（高 `10px * S`），提示玩家這隻怪物的身份。
4. **初次遭遇提示（一次性）：** 第一次靠近守護怪（距離 < `200px`）且尚未取得過鑰匙（`run._guardianHinted` 未設定），HUD 頂部短暫顯示「💡 擊敗金色守護者可獲得寶箱鑰匙！」橘色提示條，3 秒後消失，設 `run._guardianHinted = true`。

**怪物定義（`content/enemies/*.js`）：** 守護怪定義中加 `guardian: true` 旗標，`enemy.js draw()` 讀此旗標觸發上述視覺。

---

### 4.21-B 鑰匙掉落視覺強化

**目標：** 鑰匙落地後非常醒目，玩家不會走過去卻沒撿到。

**改動（`pickup.js` + `run.js`）：**

1. **掉落動畫**：守護怪死亡時，鑰匙以「彈出」動畫落地（向上飛出 `30px` 後弧形落下，落地時有 `4px` 擴散環），整個動畫約 `0.6s`。
2. **金色粒子爆炸**：掉落瞬間以守護怪位置為中心生成 `12` 個金色粒子（`world.particles` sparkle，`color: #f5c518`），向外飛散。
3. **持續閃爍光暈**：鑰匙在地圖上持續閃爍——白色底圖示（`24×24 * S`，比原來放大 1.5×），外圍每 `1.2s` 呼吸一次金色光圈（半徑 `16 → 24px * S`，透明度 `0.7 → 0`）。
4. **箭頭指引**：玩家未持有鑰匙（`world.keys < 1`）時，螢幕邊緣顯示指向最近鑰匙的金色小箭頭（與現有裝備掉落指引邏輯相同）。
5. **HUD 鑰匙計數**：撿到鑰匙後，HUD 衝刺欄旁顯示金色 🔑 `×N` 計數（`world.keys`），讓玩家隨時知道持有幾把。

---

### 4.21-C 小地圖（M 鍵）整合

- 守護怪在小地圖上以**金色菱形**（而非一般紅點）標示
- 地圖上的鎖定寶箱以**金色鎖頭圖示**標示；玩家持有鑰匙時，圖示旁顯示綠色 ✓
- 地圖上尚未撿起的鑰匙以**金色 🔑 點**標示（大於一般道具點）

---

### 4.21-D 鎖定寶箱開啟提示更新

配合以上強化，鎖定寶箱（`world.chest_locked`）的互動提示文字更新：

| 狀態 | 現有提示 | 新提示 |
|---|---|---|
| 無鑰匙 | 紅字「🔑 需要鑰匙」 | 紅字「🔑 需要鑰匙（擊敗守護者）」 |
| 有鑰匙 | 綠字「按 E 開啟」 | 綠字閃爍「✦ 按 E 開啟寶箱！」（帶金色外框脈衝） |

---

**驗證：**
- 守護怪頭頂出現金色 🔑 懸浮圖示並上下浮動；外框金色雙層輪廓；HP 條上方「🔑 守護者」標籤可見
- 初次靠近守護怪（< 200px）出現橘色提示條 3 秒；重進局後再次觸發
- 守護怪死亡後鑰匙以彈出動畫 + 金色粒子落地；地上鑰匙有呼吸金圈閃爍，比普通道具明顯更大
- 未撿鑰匙時螢幕邊緣顯示金色指引箭頭；撿起後箭頭消失、HUD 顯示 🔑 ×1
- 小地圖：守護怪 = 金色菱形、鑰匙 = 金色點、鎖定寶箱 = 金色鎖頭
- 持有鑰匙靠近寶箱，提示文字改為閃爍綠框「✦ 按 E 開啟寶箱！」

---

## 4.22 持有鑰匙道具欄顯示 + 大地圖（M 鍵）守護怪／寶箱標示（新增）

### 4.22-A 持有鑰匙的醒目道具欄顯示

**問題（4.21 的遺漏）：** 4.21 的 HUD 只在「衝刺欄旁加 🔑×N 小字」，玩家很容易忽略自己已持有鑰匙，尤其在激烈戰鬥中。

**設計：** 新增一個**局內持有道具欄（Key Item Slot）**，顯示在 HUD 右下角或衝刺欄正下方，用來展示「特殊一次性道具」（目前只有鑰匙，預留未來擴充）。
...
- 外框：金色圓角矩形（`border: #f5c518`，`border-radius: 4px`），**持續脈衝發光**（每 1.5 秒一次金色外暈，吸引注意）
- 圖示：`item_key` 或 `icon_key`（`32×32*S`），金色渲染
- 右上角小數字 `×N`（白色，`13px`）
- 道具欄只在 `world.keys > 0` 時顯示，沒鑰匙時隱藏（不佔 HUD 空間）
- **撿起鑰匙時**：道具欄從螢幕右側滑入（`0.3s ease`），同時顯示「✦ 取得鑰匙！找到金庫寶箱吧」橫幅 Toast（金色橫幅，與成就 Toast 同樣樣式，3 秒）

**玩家身上懸浮指示（世界層）：**
持有鑰匙時，玩家角色頭頂持續顯示小型金色 🔑 圖示（`12×12*S`），比守護怪的稍小，以示區別。這樣玩家在畫面中就能看到「自己頭上有鑰匙」，類似 Zelda 系列「持有道具」的直覺感受。

---

### 4.22-B 大地圖（M 鍵）上的完整標示

**問題（4.21 的遺漏）：** 4.21 只說「小地圖（minimap）」，但玩家更常用 **M 鍵大地圖**來規劃路線，守護怪和寶箱必須在大地圖上也有清楚標示。

**大地圖（`run.js` `drawBigMap()`）上新增標示：**

| 物件 | 圖示 | 大小 | 說明文字 |
|---|---|---|---|
| 守護怪（存活） | 金色菱形 ◆ + 🔑 | `10×10*S` | hover / 靠近顯示「守護者（擊殺掉鑰匙）」 |
| 守護怪（已死亡） | 灰色菱形 + ✓ | `8×8*S` | 「守護者（已擊殺）」 |
| 地上鑰匙（未撿） | 金色 🔑 大圖示 | `14×14*S` | 「鑰匙（靠近撿取）」，閃爍效果 |
| 鎖定寶箱（無鑰匙） | 金色鎖頭 🔒 | `12×12*S` | 「金庫寶箱（需要鑰匙）」，紅色邊框 |
| 鎖定寶箱（有鑰匙） | 金色開鎖 🔓 | `12×12*S` | 「金庫寶箱（可開啟！）」，綠色閃爍邊框 |
| 已開啟寶箱 | 灰色寶箱 | `10×10*S` | 「已開啟」 |
...
for (const e of world.enemies) {
  if (!e.def.guardian || e.dead) continue;
  const mx = bigMapX(e.x), my = bigMapY(e.y);
  drawMapIcon(ctx, mx, my, 'guardian_alive', S);
}
for (const p of world.pickups) {
  if (p.type !== 'key') continue;
  const mx = bigMapX(p.x), my = bigMapY(p.y);
  drawMapIcon(ctx, mx, my, 'key_pickup', S);
}
for (const c of world.chests) {
  if (!c.locked) continue;
  const mx = bigMapX(c.x), my = bigMapY(c.y);
  const hasKey = world.keys > 0;
  drawMapIcon(ctx, mx, my, hasKey ? 'chest_unlockable' : 'chest_locked', S);
}
```

**提示動線（首次開大地圖時）：**
若玩家有鑰匙但尚未打開大地圖（`run._bigMapOpenedWithKey` 未設），自動在大地圖標題旁顯示「🔑 找到金色鎖頭 🔒 即可開啟寶箱」一行提示，之後不再顯示。

---

### 4.22-C 小地圖（左上 minimap）補強

4.21 已規定小地圖的圖示，此節補充**可見性強化**細節：

- 守護怪的金色菱形圖示**持續旋轉**（每 2 秒轉一圈），在靜態的一般紅點中非常醒目
- 鎖定寶箱圖示**呼吸閃爍**（0.8s 週期）
- 地上鑰匙圖示**旋轉 + 放大縮小**（呼吸 + 旋轉組合）

---

**驗證：**
- 撿到鑰匙後：HUD 右下角出現金色邊框道具欄（大圖示 + 脈衝光暈）；同時出現「✦ 取得鑰匙！」橫幅 Toast；道具欄沒鑰匙時隱藏
- 持有鑰匙期間，玩家角色頭頂顯示小型金色 🔑
- 開大地圖（M）：守護怪為金色菱形 ◆、地上鑰匙為金色 🔑 閃爍、有鑰匙時鎖定寶箱改為綠色閃爍 🔓
- 小地圖：守護怪菱形旋轉；鑰匙圖示同時旋轉 + 呼吸；寶箱圖示閃爍
- 首次帶鑰匙開大地圖，顯示引導提示一次
...
目前魂晶（soul shard）只有一個用途：**累積 → 升級**。玩家在升級後或高速清怪時，撿到的魂晶「感覺沒有意義」，只是背景雜訊。本項目新增 4 種在局內主動或被動消耗魂晶的途徑，讓魂晶的管理本身成為一種局內決策。

現有機制（保留不變）：
- 魂晶累積滿一格 → 升級三選一（核心循環，不異動）
- 結算 `bankRun` → 溢出魂晶轉金幣（`BALANCE.SHARD_TO_GOLD`，已有）

---

### 8.4-A 魂晶共鳴（Shard Resonance）— 溢出觸發被動效果

**機制：** 每當玩家剛升完級後的 **3 秒內**，繼續撿到的魂晶視為「溢出魂晶（overflow shards）」。溢出量達 `BALANCE.RESONANCE_THRESH`（預設 15）時，自動觸發一次**共鳴爆發**，消耗這些溢出魂晶，效果由目前等級決定：

| 觸發條件 | 共鳴效果 | 實作 |
|---|---|---|
| 等級 ≤ 10 | 以玩家為中心 120px 半徑爆炸，傷害等於當前武器最高 dps × 0.5 | `world.dealAreaDamage` |
| 等級 11–20 | 玩家獲得 3 秒「魂晶護盾」，免疫下一次傷害（一次性，顯示藍色外環） | `player._soulShield = 3` |
| 等級 21+ | 玩家隨機傳送至地圖中安全位置（半徑 > 150px 無敵人） | `player.teleportSafe(world)` |

**`balance.js` 新增常數：**
```js
RESONANCE_THRESH: 15,     // 溢出多少魂晶觸發共鳴
RESONANCE_WINDOW: 3,      // 升級後幾秒內的溢出才算（秒）
```

**`run.js` 實作位置：** `progressTick()` 內，升級後設 `run._resonanceWindow = BALANCE.RESONANCE_WINDOW`；`world.js pickupShard` 時若 `_resonanceWindow > 0` 則計入 `run._overflowShards`，達閾值後呼叫共鳴效果。

**HUD 表示：** 升級後的 3 秒「共鳴窗口」內，XP 條邊框變藍色脈衝，提示玩家此時撿到的魂晶有額外效果。

---

### 8.4-B 魂晶急救（Shard Mend）— 危急時消耗魂晶換 HP

**機制：** 當玩家 HP ≤ 15%（`BALANCE.MEND_HP_THRESHOLD`）時，地圖上的**魂晶祭壇**（現有 altar 互動物，`world.js` interactables）出現「⬆ 魂晶急救」提示。按 `E` 互動後：

- 消耗 `BALANCE.MEND_SHARD_COST`（預設 30）魂晶（從目前 XP 進度扣除，不跌等）
- 回復 `BALANCE.MEND_HEAL`（預設 25%）最大 HP
- 同一祭壇本局只能用 1 次（防止反覆刷）

若魂晶進度不足 30，按鍵無效並顯示紅字「魂晶不足」。

**`balance.js` 新增常數：**
```js
MEND_HP_THRESHOLD: 0.15,  // 低於最大 HP 幾 % 才啟用
MEND_SHARD_COST: 30,       // 消耗魂晶（XP 進度）
MEND_HEAL: 0.25,           // 治療量（佔最大 HP 比例）
```

**`world.js` 實作：** altar interactable 的 `onUse` 回呼中檢查 `player.hp / player.stats.maxHp <= MEND_HP_THRESHOLD` 才顯示此選項；使用後設 `altar._mendUsed = true`。

**HUD 提示：** 危急狀態下的最近祭壇在地圖上顯示白色閃爍光點，`hud.js` 在 HP 條下方顯示「⬆ 附近有急救祭壇 [E]」小提示。

---

### 8.4-C 魂晶充能（Shard Charge）— 主動技能消耗

**機制：** 按 `Q`（可於按鍵設定 4.xx 重設）消耗**目前 XP 進度 × 40%**（不跌等，最少消耗 10），換取 **4 秒充能狀態**：
- 傷害 +30%
- 攻速 +20%
- 玩家角色有橘色能量外框（視覺）

冷卻時間 `BALANCE.CHARGE_COOLDOWN`（預設 20 秒），冷卻期間 Q 無效。充能期間按 Q 無效（不疊加）。

**`balance.js` 新增常數：**
```js
CHARGE_XP_COST_RATIO: 0.4,  // 消耗目前 XP 進度的比例
CHARGE_XP_MIN: 10,           // 最低消耗量
CHARGE_DURATION: 4,          // 充能持續秒數
CHARGE_DMG_BONUS: 0.3,       // 傷害加成
CHARGE_ATK_BONUS: 0.2,       // 攻速加成
CHARGE_COOLDOWN: 20,         // 技能冷卻（秒）
```

**`run.js` / `player.js` 實作：**
```js
// run.js handleKeys()
if (e.code === 'KeyQ' && !run._chargeCooldown) {
  const cost = Math.max(BALANCE.CHARGE_XP_MIN,
    Math.floor(run.xp * BALANCE.CHARGE_XP_COST_RATIO));
  if (run.xp >= cost) {
    run.xp -= cost;
    player._chargeActive = BALANCE.CHARGE_DURATION;
    run._chargeCooldown = BALANCE.CHARGE_COOLDOWN;
  }
}
...
if (this._chargeActive > 0) {
  ...
  effectiveDmg *= (1 + BALANCE.CHARGE_DMG_BONUS);
  effectiveAtkSpd *= (1 + BALANCE.CHARGE_ATK_BONUS);
}
```

**HUD 表示：** Q 技能圖示顯示在 HUD 左下角（與衝刺欄同區），含冷卻扇形遮罩 + 充能狀態橘色 glow。

---

### 8.4-D 魂晶轉化商店（Shard Exchange）— 局內特殊購買

**機制：** 局內的**商店（B 鍵，`run.js` shop）** 新增「魂晶換取」分頁 / 區塊，玩家可選擇用魂晶（XP 進度）而非金幣購買特定物品：

| 魂晶商品 | 消耗 | 金幣等效 | 限購 |
|---|---|---|---|
| 治療藥水（回 20% HP） | 25 魂晶 | — | 每局 3 次 |
| 隨機被動技能（從池中抽 1） | 80 魂晶 | 不可金幣購買 | 每局 1 次 |
| 鑰匙（開鎖寶箱） | 40 魂晶 | — | 每局 2 次 |
| 重置本次升級選擇（Reroll） | 15 魂晶 | — | 每次選擇 1 次 |

**注意：**
- 魂晶消耗從 `run.xp` 扣除，不跌等（進度最低止於 0）
- 「重置升級選擇」商品只在有 `run.choice` 時出現（在三選一面板時按 B 才顯示此項）

**`run.js` shop 渲染修改：** 在現有商店底部新增「💎 魂晶市集」分隔線，列出上表的可用商品，各自顯示「💎 消耗 N 魂晶」而非金幣圖示，目前庫存（剩餘可購次數）標在右側。
...
| 途徑 | 消耗來源 | 好處 | 代價 |
|---|---|---|---|
| 共鳴（A） | 溢出被動觸發 | 免費效果 | 需剛升完級時快速撿取 |
| 急救（B） | XP 進度（-30） | 緊急回血 | 升級變慢、需靠近祭壇 |
| 充能（C） | XP 進度（-40%） | 4 秒爆發 | 升級變慢 + 20 秒冷卻 |
| 轉化商店（D） | XP 進度（-15~80） | 靈活購買 | 升級次數減少 |

核心循環（積魂晶升級）仍是主幹，四種途徑皆是**選擇性消耗**，不強制使用。

---

**驗證：**
- 升級後 3 秒內快速撿魂晶，達 15 個後觸發對應等級的共鳴效果；XP 條邊框在共鳴窗口期間顯示藍色脈衝
- HP ≤ 15% 時靠近祭壇顯示急救提示；魂晶不足時紅字提示；使用後 HP 回升 25% 且同祭壇無法重複使用
- 按 Q 消耗 XP 進度（40%），進入 4 秒充能（橘色外框）；再按 Q 無效；20 秒後冷卻結束
- 商店（B）底部「魂晶市集」列出商品；購買後 `run.xp` 正確扣除（不跌等）；各商品限購次數正確遞減
- 以上四種途徑均不強制，不影響原始「積滿升級」核心流程
...
- 魂晶共鳴：升級後 3 秒共鳴窗口（XP 條藍框）；溢出 15 魂晶後依等級觸發爆炸 / 護盾 / 傳送
- 魂晶急救：HP ≤ 15% 時祭壇出現急救選項（E）；消耗 30 魂晶回 25% HP；同祭壇限 1 次；魂晶不足紅字提示
- 魂晶充能（Q）：消耗 XP 進度 40%（最少 10），4 秒橘色爆發（傷害+30%/攻速+20%），20 秒冷卻；HUD Q 圖示含扇形冷卻
- 魂晶商店分頁：B 鍵商店底部「💎 魂晶市集」，4 種商品（藥水/隨機被動/鑰匙/Reroll）；購買扣 XP 進度不跌等；各限購次數正確
...
- 魂晶使用途徑多樣化（4 種新途徑）：共鳴（升級溢出觸發爆炸 / 護盾 / 傳送）、急救（HP ≤ 15% 消耗 30 魂晶回血）、充能（Q 鍵消耗 40% XP 換 4 秒爆發）、魂晶商店（B 鍵新「魂晶市集」分頁，4 種商品）；核心升級循環不受影響，均為選擇性消耗。
