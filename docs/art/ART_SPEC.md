...
| 級距 | 畫布 | 可視高度（實心像素） | 適用 |
|---|---|---|---|
| 英雄 | 16×18，anchor feet `[8,17]`，4 frames | 17–18 px（含 1px outline；gate 實測全 27 名一致） | 全部 27 角色（現制不變） |
...
規則：
- **同級距內可視高度偏差 ≤2 px**（contact sheet 同尺度並排驗收）。W3-A1 放寬（2026-08-27）：此項為指引——造型自然佔高者（鐘罩/持袋人形等）允許超出，硬約束是畫布級距＋填充率 ≥55%。
...
- **剪影可讀性（R29 補寫，經 27 人重切實證）**：
  - `Painter.outline()` 把 **alpha > 0 一律視為實心**——`glow/aura/ring/star4/softShadow` 的低 alpha
    光暈會被描進外輪廓，使剪影變成「發光半徑」而非身體。**角色身上的發光半徑必須 ≤1.6**，
    否則該光暈就是你的輪廓。（R28 靠拉高填充率完全沒改善輪廓，根因即此。）
  - `outline()` 會填死 1 px 縫隙：突出物（武器/角/披風/傀儡）要在剪影上與軀幹分離，
    中間**至少留 2 個空白欄，≥3 才保險**。
  - 驗收看**黑剪影對照表**（去色、僅 alpha>128），不是看填充率——填充率達標 ≠ 輪廓達標。
    同類職業（弓手/法師/重甲）必須有互不重疊的**頂部剖面**。
...
> **鐵律（2026-08-27 由 W3-B1/B2 退回事故補寫，違反即整批退回）：類別文法只活在「框」與
> 次要提示上；glyph 必須畫出「那一件具體的東西」，且同類別內任兩個 glyph 的輪廓不得雷同。**
> 失敗樣態（實際發生過）：把全部裝備畫成同一支斜桿加色頭、全部道具畫成同一個框箱、全部設施
> 畫成同一頂帳篷——只靠顏色區分＝ART-06 原病症復發，比改之前更糟。
> 執行要求：動筆前先列「id → 這張畫的是什麼具體物件」計畫表並自查重複，通過才畫。
...
- 左上 kira 星：**只給進化武器（含 w_soulstorm/w_inferno 這類不以 `_evo` 命名者）、epic 裝備
  `ep_*`、隱藏獎勵 `hr_*`**——2026-08-27 收緊：先前按 def.tier≥2 判定造成 121/219（55%）都帶星，
  kira 自己變成新的同質裝飾；現為 22/219（10%），恢復「少數特殊」語義。專屬武器裝備 `x_*`
  是每個英雄的常規解鎖，不標。
