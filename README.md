...
| 自動武器（含進化合成；R17 新增隱藏密室專屬「禁書迴響」） | **31** |
| 可玩角色（含隱藏英雄） | **15** |
| 被動能力（含 **詛咒強化** + 異常狀態被動；R17 新增「聖物之心」） | **54** |
| 裝備（含史詩/稜彩；R17 新增傳說飾品「寶庫之印」） | **41** |
| 道具（含能力值鐵砧） | **28** |
| 天賦 / 設施 | 20 / 11 |
| 生態（各具**獨特地貌**＋專屬裝飾，非換色換皮；R9 新增**翠林/荒漠/腐沼/深淵/天界**） | **10** |
| 成就 / 故事章節 | **205 / 19** |
| 羈絆 · 角色專屬武器 · 贊助者事件 | **12 · 8 · 16** |
| 角色造型（含 **5 款隱藏全身造型**，含彩蛋專屬「開發者·小妖」） | **16** |
| 一般 / 隱藏任務 | **6 / 3** |
| 錄製配樂曲目 | **12** |
...
- 城鎮永久成長（天賦樹 + 設施，**消費隨購買次數動態漲價**）、可互動的**任務公會（接取一般/隱藏任務、前置任務鏈、可同時追蹤多個）/ 成就殿堂（全部・已達成・未達成 篩選）/ 衣帽店（**R17 改版：入口分「我的造型／造型商店」，商店每 30 分鐘隨機進貨 8 款（已解鎖角色×造型配對、1% 隱藏款），普通 1000／豪華 3000／隱藏 20000–50000，每週特賣 8 折不含隱藏款**）/ 魂晶銀行（**自訂金額借款，下一局結算自動連本帶利還款**）建築**、**追蹤任務常駐螢幕左側（多列堆疊）**、**成就解鎖內容**、**開場故事過場 + 新手教學**（城鎮嚮導 + 首局戰鬥提示 + HUD 導覽）、選角頁顯示英雄能力差異、金幣經濟
- **玩家回報系統**：城鎮 Esc 選單可送出問題回報／建議（分類 + 描述 + **截圖附件**：Ctrl+V 貼上 / 選檔 / 拖放），未登入也能送；管理者後台主控台（7 分頁：總覽 / 玩家 / 對局 / 廣播 / 回饋 / 稽核 / 統計）可檢視回饋附圖、封鎖帳號/IP、踢人、廣播走馬燈、稽核日誌與數據統計
...
npm test   # 後端測試（帳號/存檔/排行榜/回饋/後台 + 好友/房間/中繼，共 92 + 65 項）
```
