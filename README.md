...
> **V2.1 · R32**：合作對局改由伺服器執行世界、升級與結算，改善房主離場、斷線重連及儲存狀態提示。成長診斷與 8100 局 A/C 確認實驗完成，保留策略 A；真人評估仍待資料。版本紀錄見 [ROUND32](docs/changelog/ROUND32.md)；正式站版本以遊戲標題畫面為準。
...
- 🎨 **遊戲內美術 100% 程式即時生成**(全部 sprite 無外部圖片);唯一的圖片資產是 R28 加入的 27 張角色半身肖像 `assets/portraits/`(僅用於選角卡與圖鑑,同源、載入失敗自動回退為程式生成 sprite);音效 WebAudio 合成、配樂為 12 首實錄曲目(依遊戲狀態交叉淡入,合成器為後備)
- 🕹️ **可純單機離線遊玩**:雲端與多人皆為選配,未登入／連不上自動退回本機存檔,不影響遊玩
- ⚙️ **前端零安裝**:一個靜態伺服器即可執行;**執行期不對任何第三方主機發出請求**(字型已自架,見[隱私](#匿名遊玩統計遙測))

**版本沿革**：目前程式版本 **V2.1（R32）**；V1.0=R18、V1.5=R19、V2.0=R20，R21–R31 為內容、品質與策略研究輪。已實作末日遺跡城鎮、無盡／每日／週常模式、資料驅動 Boss 招式與局內事件、全 27 角色專屬武器、內容圖鑑與推薦目標、無障礙與輔助模式、匿名遙測、結算教練、場景光影美術輪。R27 為外部程式碼審查的修正批次(排行榜誠信閘門、雲端存檔資料遺失、字型自架、相依清零),不含新玩法。R28 為外部美術審核的改善輪:全角色與敵人尺度統一、戰鬥分層與預警所有權、UI 字級系統、10 生態地標與材質身份、27 張角色肖像、219 個圖示重繪、六室內改造,同樣不含新玩法。R29 為複審跟進輪:27 角色剪影重切(無名稱可辨)、數字不斷行、肖像三曝光面、三檔面板密度、色覺形狀通道、玩家標識暗通道、高 DPI 公平性(zoom/字級/1px 線)。R30 完成 CI／Node 維護與機器人平衡測試工具、全量 6750 局摘要；不更動遊戲數值或玩法。R31 的 A/B 與 R32 的 A/C 實驗皆保留基準 A；R32 完成成長診斷與伺服器權威合作，真人評估尚未完成。逐輪更新詳見 [`docs/changelog/`](docs/changelog/)（最新 [`ROUND32.md`](docs/changelog/ROUND32.md)）。
...
雲端與多人是**選配且離線優先**:沒登入或連不上時自動退回本機 `localStorage`,單機體驗完全不受影響。接上後端後,可獲得帳號、跨裝置雲端存檔、伺服器權威計分的共享排行榜,以及 **1~3 人即時連線合作**（V2.1 採伺服器權威模擬）。

## ✨ 技術亮點

- **零建置純前端**:原生 HTML5 Canvas + ES Modules,無打包器、無框架、無執行期依賴;`node tools/serve.mjs` 即可開跑。
- **100% 程序化像素美術**:全部 sprite 由程式以共用 `Painter` 像素繪圖 API + 調色盤即時生成並快取;敵人、武器、角色、城鎮、生態地貌皆走同一套風格管線。(唯一例外是 R28 的角色肖像,見上;它不參與 sprite 管線,只在 UI 卡片上顯示。)
...
- **CI 與離線優先設計**:GitHub Actions 跑 185 項後端 smoke/social 測試、權威模擬測試組與 59 條前端 headless smoke 斷言,兩者由 `deploy.yml` 的 `needs: [test, frontend]` 組成**部署雙關卡——任一紅燈就不部署**;前端另附離線自測 hook(`__DBG.coopRoundTrip/coopSilenceTest/coopBossSyncTest`)可在無雙分頁、無中繼伺服器的情況下驗證 host→guest 全鏈路。
...
| `空白` | 城鎮中快速開啟出擊面板 |
| `Esc` | 單機暫停／設定；合作中開啟選單但世界繼續；城鎮中為帳號／多人／排行榜／回報選單 |
...
- **即時合作（V2.1）**：沿用好友／大廳／邀請／觀戰入口，開局至少兩人、最多三人。伺服器執行世界，房主離開後隊伍繼續；斷線角色仍承受傷害，重連沿用當前生命與死亡狀態。先同步原房主存檔；雲端歷史進度本身不宣稱已驗真。
...
# 後端測試（120 項 smoke + 65 項 social + 權威模擬測試組）
npm test
```
...
src/
  ...
  game/
    ...
    scene.js  scenes/           title / hub(城鎮) / run(局內) / coop(合作快照呈現)
...
server/                         ← 後端:REST API + /rt 權威合作
  src/authority/                每房間隔離模擬、輸入驗證、唯一結算
...
assets/portraits/               ← R28 的 27 張角色半身肖像(128×128 PNG,選角卡/圖鑑用)
```
...
- **合作開局 2~3 人**：鏡頭各自跟隨，同一張世界地圖；隊友離開後可剩一人繼續。預設最多同時 2 間模擬房；正式 ARM 主機與晚期怪潮容量尚待量測。
- **合作結算與儲存**：永久進度仍歸原房主。勝負先顯示，雲端儲存另行確認；儲存衝突不覆寫較新進度，尚未提供自動合併。伺服器重啟不會恢復進行中的世界。
...
> `assets/portraits/` 的 27 張角色肖像以生成式模型產出概念稿後,再經人工挑選方向、統一色盤、
> 降採與系列一致性處理而成(流程與原稿見 [`docs/reviews/art-improve-2026-08/`](docs/reviews/art-improve-2026-08/));
> 若你要重散布或商用,請自行確認生成式素材在你所在地的授權與合規要求。
...
| [`docs/changelog/`](docs/changelog/) | 逐輪更新紀錄(版本記事唯一來源,最新:`ROUND32.md`) |
...
已完成 540 局診斷，涵蓋 A/B 的 XP 掉落／收集、選項供給與升級節奏，最終 error 與 XP 守恆違例皆為 0；結果見 [成長診斷摘要](docs/reviews/bot-balance/2026-09_growth-diagnostic.md)。診斷沒有重新採用 B。既有 R31 輸出不可混用，重現實驗須使用原 manifest 指定的固定來源；不可用本次版本接續舊批次。

真人評估的 [流程與紀錄表](docs/research/r32-human-evaluation/PROTOCOL.md) 已備妥，目前沒有受測資料。退出／重試意願、短局模式、手動瞄準與 co-op UX 尚無真人結論；短局與手動瞄準仍是概念評估，尚無可玩原型。
...
確認批次已完成 **8100 唯一 key**（原始 8104 行、最終 error 0）。A/C 各通關 65 局，有效通關率 1.6193%／1.6177%；差異 −0.0016 個百分點，95% CI [−0.5232, +0.5201]，未達採用門檻，**保留 A**。C 提高早期 XP 拾取，未改善通關與中位存活。詳見 [A/C 報告](docs/reviews/bot-balance/2026-09_strategy-ac.md)。測量的是系統性質，非真人行為；遊戲 RNG 不受控，固定規則策略的結果不能代表真人通關率。
