...
也可把滑鼠移到 HUD 或 build 頁的武器 / 被動 / 裝備 / 道具圖示上查看說明。每位角色可獲取的武器/被動有上限（武器 6、被動 14）。
...
2. **單一關卡（局內）**：一局＝一張**超大生態地圖、限時 30 分鐘**。**威脅等級隨時間爬升**（1→13，影響怪物階級與強度），**難度**再乘上整體強度；同一時間只出 **1~3 種怪物**並隨時間輪替；地圖散布**陷阱地形**（熔岩 / 尖刺 / 毒沼 / 荊棘，對雙方都有傷害）。期間有**週期首領事件**與**特殊事件**（提摩蘑菇、縮圈、希格斯炸彈大絕）與**特殊怪物**（自爆炸客…）。
3. **最終決戰**：第 26 分鐘起**全圖縮圈**逼向中心，第 30 分鐘該關**最終首領**降臨。**擊敗即破關** → 解鎖**下一關卡**與**本關更高難度**，並給予大量金幣。
4. 升級三選一：**新武器 / 武器升級 / 被動**（卡片依稀有度上色並標示數值）。武器滿級且有對應被動可**進化合成**；也可選**武器融合**（犧牲一把使另一把直接滿級進化）。**詛咒強化**提供強力效果但附帶負面代價。走到**魂晶商店**買史詩/稜彩裝備與**能力值鐵砧**；整局不取裝備、專買鐵砧者，有機會觸發一次性**稜彩祝福**（隨機 +1~100%，自此走純能力值之道）。
...
- 走位自動攻擊、多武器同時開火、武器升級 / 進化合成 / **武器融合**；攻速由慢到快隨局內時間提升
- 多角色（被動 + 起始武器 + 解鎖條件）、隱藏英雄
- **關卡制**：限時 30 分鐘、時間驅動威脅等級、**難度分級**、**破關解鎖下一關與更高難度**
- **最終縮圈 + 關卡最終首領**；期間 1~3 種怪物輪替、陷阱地形、週期首領、特殊事件/怪物
- **詛咒系統**：強力被動附帶負面代價；卡片依稀有度上色並標示數值
- **局內魂晶商店**：史詩/稜彩裝備、**專武（裝備後成為自動開火的招牌武器）**、能力值鐵砧（仿 LoL 海克斯競技場），與隱藏「純能力值」一次性稜彩祝福
- 城鎮永久成長（天賦樹 + 設施）、**成就系統**、**故事任務鏈**、金幣經濟（弱化局內、強化局外）
- 程序合成音樂（地圖/英雄/首領變化）、可調震動（瀕死才劇烈）
- **120 Hz 模擬**（低輸入延遲）、設定選單（音量/震動/靜音/重置存檔）、排行榜、小地圖、HUD 提示與 build 檢視頁
