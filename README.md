...
1. **城鎮（Hub）**：可走動的營地。用金幣在「天賦祭壇」買 4 系永久天賦、在「設施工坊」蓋設施、查看「成就殿堂」與「故事任務」（領取章節獎勵）；走向「傳送門」**選擇角色**出擊（角色用金幣或達成成就解鎖，含隱藏英雄）。
2. **生存（局內）**：在**單一張超大生態地圖**中走位求生 —— 這一張地圖就是整局戰場。**威脅等級隨時間提升**（影響怪物階級與強度）；同一時間只會出現 **1~3 種怪物**，並隨時間輪替。地圖上散布**陷阱地形**（熔岩 / 尖刺 / 毒沼 / 荊棘，對雙方都有傷害）。**首領以事件方式週期登場**（隨機挑選、多階段、混合彈幕與放射攻擊）。
3. **特殊事件**不時來搞你：提摩蘑菇地雷、縮圈、希格斯炸彈大絕。**特殊怪物**（自爆炸客、噬魂蚊群、噴酸蛞蝓…）混入潮汐。
4. 升級可三選一：**新武器 / 武器升級 / 被動**；武器滿級且擁有對應被動可**進化合成**。走到**魂晶商店**可買史詩/稜彩裝備與**能力值鐵砧**；整局不取裝備、專買鐵砧者，有機會觸發一次性的**稜彩祝福**（隨機 +1~100%，自此走純能力值之道）。
5. **死亡**：結算分數並進入排行榜，帶回的金幣存入金庫用於永久成長、解鎖成就與推進故事。越死越強。
...
- 走位自動攻擊、多武器同時開火、武器升級與進化合成；攻速由慢到快隨局內時間提升
- 多角色（被動 + 起始武器 + 解鎖條件）、隱藏英雄
- **單張超大地圖**的持續生存：時間驅動的威脅等級、1~3 種怪物輪替、陷阱地形
- **首領事件**（隨機、多階段、混合彈幕/放射）、**特殊事件**（蘑菇/縮圈/炸彈大絕）、**特殊怪物**（自爆等）
- **局內魂晶商店**：史詩/稜彩裝備、能力值鐵砧（仿 LoL 海克斯競技場），與隱藏的「純能力值」一次性稜彩祝福
- 城鎮永久成長（天賦樹 + 設施）、**成就系統**、**故事任務鏈**、金幣經濟
- 程序合成音樂（地圖/英雄/首領變化）、可調震動（瀕死才劇烈）
- 設定（音量/震動/靜音）、暫停選單、分數排行榜、小地圖、HUD 提示與 build 檢視頁
