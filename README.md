...
**目前進度**：Round 17 完成；Round 18 規格已定稿（[`docs/ROUND18_SPEC.md`](docs/ROUND18_SPEC.md)）。逐輪更新詳見 [`docs/changelog/`](docs/changelog/)。
...
1. **城鎮（Hub）**——可走動的多房小鎮：教堂買天賦、鐵匠鋪鍛造武器與蓋設施、公會接任務與升聲望、衣帽店買造型、成就殿堂與個人小屋查看進度，加上 10 位可對話 NPC。走向傳送門開啟**出擊面板**：選角色 × 生態 × 難度（劇情～D5＋無盡）。
2. **一局出擊**——一張超大生態地圖（138×102）、限時 **20 分鐘**。威脅等級隨時間 1→約 13 爬升，同時段只出 1~3 種怪並輪替；地圖散布陷阱地形、互動物、**隱藏房間**（一次性永久獎勵）與**守護怪＋鑰匙＋寶庫**。每 **5 分鐘**降臨一隻各不相同的**小王**（擊殺掉落贊助者事件三選一），期間穿插特殊事件（怪物包圍縮圈、希格斯炸彈雨、提摩蘑菇…）。
...
- **收集**：205 項成就（許多綁內容解鎖、含隱藏成就與專屬篩選）、19 章故事任務、角色造型商店（每 30 分鐘隨機進貨、1% 隱藏全身造型、每週特賣）
...
| 類別 | 數量 |
| --- | --- |
| 敵人（含 8 名多階段首領＋隱藏死神＋特殊怪物） | **48** |
| 自動武器（含進化合成與隱藏房專屬） | **31** |
| 可玩角色（含隱藏英雄） | **15** |
| 被動能力（含詛咒強化與異常狀態被動） | **54** |
| 裝備（含史詩／稜彩與傳說飾品） | **41** |
| 道具（含能力值鐵砧） | **28** |
| 生態系（各具獨特地貌與專屬裝飾，非換色換皮） | **10** |
| 成就 / 故事章節 | **205 / 19** |
...
# 後端測試（帳號/存檔/排行榜/回饋/後台 + 好友/房間/中繼，共 92 + 65 項）
npm test
```
...
src/
  ...
  art/                          程序化美術（sprite 定義；town_*/biome_* 各主題分檔）
  ...
  game/
    ...
    world.js  maps.js  floor.js 世界（磚塊碰撞/實體/戰鬥；players[] 多人化）與地圖生成
...
| 文件 | 內容 |
| --- | --- |
| [`CLAUDE.md`](CLAUDE.md) | 開發指南：架構、慣例、Gotchas、測試 |
| [`docs/changelog/`](docs/changelog/) | 逐輪更新紀錄（最新：`ROUND17.md`） |
| [`docs/ROUND18_SPEC.md`](docs/ROUND18_SPEC.md) | Round 18 實作規格（已定稿、待實作） |
| [`docs/ROUND17_SPEC.md`](docs/ROUND17_SPEC.md) | Round 17 實作規格（已完成） |
