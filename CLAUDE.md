...
- **Known limitations (round-6 QA, intentionally deferred — see ROUND6.md §6):** the forge **疾速/swift** effect does nothing for *update-driven* weapons (beams/auras/turrets self-gate via their own timers, not the cooldown branch); and enemy separation is **O(n²)** at the 260-enemy cap with no spatial grid (fine on the happy path — a future perf optimization, add a uniform grid + forEachNear if frames dip).
