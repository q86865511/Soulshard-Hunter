...
- [x] T10 [NORMAL][免TDD:實跑產出] 首份全量批次：`--biomes all --chars all --diffs 1,2,3,4,5 --runs 5 --parallel 4`（可分片、可續跑），完成後把 report.md 的決策摘要（異常格、角色極差、diff1 合理性檢查）寫進 `docs/reviews/bot-balance/2026-09_summary.md`（對應 R6、R8；驗收：`runs.jsonl` 行數＝6750（或分片聯集）、摘要檔存在；依賴：T8）
