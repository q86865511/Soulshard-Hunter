...
當一個組合被指派給頁面時，系統應以 `newRun({biomeId, characterId, difficulty, mode:'normal'})` 建局、掛上 run scene，在 pump 迴圈中每格清除全部暫停來源、把各種選擇交給策略，直到 `scene.dead` 為真或模擬時間超過上限 `LEVEL_TIME + REAPER_DELAY + 120s`。
...
- `choice`/`equipChoice`/`eventChoice`/`curseChoice`/`shopChoice` 出現後在下一格前被清除（不卡局）。
...
- level：其餘情況優先序＝已持有武器升級（等級低者先）＞新武器＞被動＞其他；同序以 `seed` 決定 tie-break，同 seed 同結果。
- equip：同槽比 tier 高者；武器槽裝備（signature）一律不選（避免改變基準武器）；options 空 → `-1`。
- event / curse：回傳 `0`（固定第一項，報告中註明）；shop：回傳 `-1`。
- options 空或 kind 未知 → `-1`，不拋例外。

### R5 每局紀錄

當一局結束時，系統應在 `runs.jsonl` 追加一行 JSON，欄位：`biome, char, diff, mode, runIdx, seed, result, time, level, kills, score, stage, deathSrc, dmgTakenBySrc(前 5)、dmgBySource(前 5)、weapons[], abilities[], bossKills, reaperSlain, hpTimeline(每 30s 的 hp 比例陣列), simMs, ticks, error(選填), toolVersion`。

驗收條件:
- `validateRecord(obj)` 對合法紀錄回 `[]`；缺 `result` 或 `time` 非數字 → 回錯誤欄位名陣列。
- `result:'timeout'`/`error` 局也寫一行，`time` 為當時模擬秒數。
...
驗收條件:
- 固定 fixture（6 筆，含 2 生態 2 角色）→ 輸出含指定的表格列字串與異常項。
- 空陣列 → 輸出含「無資料」且不拋例外。
- `timeout`/`error` 局計入 n 但不計入通關率分母（報告註明筆數）。
...
系統應以 `--parallel P` 同時驅動 P 個頁面，各頁面依序取下一個未跑組合；每局記錄 `simMs`（實際耗時）與 `ticks`；`--dry-run` 的預估時間以 `specs/bot-balance-sim/BENCH.md` 記錄的基準（單局 simMs 中位數）計算。因全量規模（6750 局）補充的設計元素：續跑（重啟時讀既有 `runs.jsonl` 跳過已完成組合）與分片（`--shard i/n`）。
...
系統應讓策略層在同一 `seed` 下對同輸入產生同輸出；遊戲本身的 `Math.random` 不可控，工具在報告與 JSONL 的 `toolVersion` 旁明記「遊戲 RNG 不受控，局間差異屬正常」。
